...
const NPCS: NpcDef[] = [
  { name: '田中PM',   col: 2,  row: 1, lines: ['いよいよ最終章だ', '本番環境へのデプロイ準備をしてください', 'スケジュールは死守、品質は…まあなんとかなる', 'リリースさえすればこっちのもん。あとは運用で', 'よし、頼んだぞ！なんとかなるなる！', '最初に「気合いと根性でなんとかなる」って言ったのは俺だけど、まさかほんとになるとはね（笑）', '佐藤くんも「あいつは伸びる」って最初から言ってたよ。俺も同感'] },
  { name: '佐藤先輩', col: 9, row: 3, lines: ['デプロイ手順、確認した？', '不安なところがあれば一緒に見るよ', '表向きは『手順書通りに』なんだけど', '実際は『何かあったら呼んで』が本音かな', '最初「分からなかったらいつでも聞いてね」って言ったけど、もう聞くことなくなっちゃったね', '田中さんが「なんとかなるなる」って言い続けた先に、本当になんとかなったの、ちょっと悔しい（笑）'] },
  { name: '鈴木さん', col: 14, row: 9, lines: ['運用保守って、終わりがないんですよ…', '運用フェーズになると、急に人減らされるんですよ', 'リリースしてもまた次の改修が始まりますからね', '客先常駐のまま何年目になるかな…', 'でも、ここまで一緒にやれて嬉しかったです', '最初「指示系統がややこしい」って説明したのに、今は自然と動けてますよね。すごいです', '三次請けのこと、笑って話せるようになったのは、このチームのおかげかもしれないです'] },
];
...
const CHOICES: Record<'deploy' | 'incident', Choice[]> = {
  deploy: [
    { text: '手順書通りに慎重にリリース',         score: 10, result: '手順書を一つずつ確認しながら慎重にリリース。本番は何のトラブルもなく稼働し、佐藤先輩も「完璧だね」と笑顔だった。田中PMが「なんとかなった！（笑）」と言ったが、今日ばかりはツッコめなかった。[+10点]' },
    { text: '急いでそのままデプロイ',             score: -5, result: '早く終わらせたくて手順を一部飛ばしてデプロイ。直後にエラーログが大量に出力され、緊急対応に追われることに。佐藤先輩に「最後くらい手順通りにしようよ…」と苦笑いされた。[-5点]' },
    { text: '先輩とダブルチェックしてリリース',   score:  5, result: '佐藤先輩とダブルチェックしながらリリース。二人で確認したことで安心感もあり、無事に本番稼働できた。鈴木さんも「チームで確認、一番ですよ」と言ってくれた。[+5点]' },
  ],
  incident: [
    { text: 'ログを確認して原因を特定',           score: 10, result: 'ログを丁寧に確認し、原因の処理を特定。迅速に修正パッチを適用し、影響を最小限に抑えることができた。佐藤先輩が「最初の頃から比べたら、見違えるほど頼もしいよ」と言ってくれた。田中PMも「なんとかなるって信じてたよ！」と笑った。[+10点]' },
    { text: 'とりあえずサーバー再起動',           score: -5, result: 'とりあえず再起動したら一時的に直ったように見えた。だが30分後に同じ障害が再発し、結局原因調査をする羽目に。鈴木さんが「うちも同じことやったことあります…根本原因、大事ですよね」と同情してくれた。[-5点]' },
    { text: '影響範囲を確認してから対応',         score:  5, result: 'まず影響範囲を確認し、関係者に連絡してから対応。落ち着いて行動できたことで、混乱を最小限に抑えられた。佐藤先輩が「最初のころと比べて、ちゃんと段取りができるようになったね」と言ってくれた。[+5点]' },
    { text: '関係者全員に「障害発生」とだけ一斉送信する', score: 0, result: '急いで関係者全員に「障害発生」と一斉送信したが、詳細が分からず「状況は？」という問い合わせが次々と来てしまった。「まず自分で確認してから連絡しよう」と佐藤先輩にアドバイスされた。田中PMが「まあ、なんとかなるよ！」と言っていたがそれどころではなかった。[±0点]' },
  ],
};
...
export class Chapter7Scene extends Phaser.Scene {
  ...
  private getLines(npc: NpcDef): string[] {
    if (npc.name === '田中PM') {
      ...
      if (this.gameStep === 1) return ['リリース判定会議は通った。準備は万全かな', '何か不安なことがあれば、今のうちに言ってね', '佐藤くんも鈴木さんもスタンバイOKだから、安心して', '机に戻って、最終チェックをお願い'];
      return ['本番切替、緊張するね。落ち着いていこう', '何かあっても、みんなでなんとかするから', 'あの頃から比べると、見違えるほど頼もしくなったよ', 'いよいよだね、頑張ろう'];
    }
    if (npc.name === '佐藤先輩') {
      if (this.gameStep < 2) return ['デプロイ手順、確認した？', '不安なところがあれば一緒に見るよ', '表向きは『手順書通りに』なんだけど', '実際は『何かあったら呼んで』が本音かな'];
      if (this.gameStep === 2) return ['本番から障害報告が来た！対応お願い', '切り戻すかどうかも判断してね', '田中さんは「なんとかなる！」って言ってるけど（笑）', '机に戻って対応しましょう'];
      return ['対応お疲れ様。よく頑張ったね', 'この一年で、すごく成長したと思うよ', '胸を張っていいからね', '田中さんが「次のプロジェクトも頼む」って言ってたよ', '次の新人さんが来たら、今度はあなたが教える番かもね'];
    }
    if (npc.name === '鈴木さん') {
      ...
      return ['お疲れ様でした！', 'これで一区切りですね。本当によく頑張りました', '正直、最初はどうなることかと思いましたよ', '佐藤さんも田中さんも、このチームが好きでした', '三次請けでも、こんなチームに出会えてよかったです', 'また一緒に仕事できたら嬉しいです'];
    }
    return npc.lines;
  }
  ...
  private getChoices(key: 'deploy' | 'incident' | 'flareup'): Choice[] {
    return key === 'flareup' ? FLAREUP.choices : CHOICES[key];
  }
  ...
  private showChapterClear() {
    ...
    const total = getTotalScore(this.diffLevel);
