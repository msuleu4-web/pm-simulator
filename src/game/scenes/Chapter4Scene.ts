...
const NPCS: NpcDef[] = [
  { name: '田中PM',   col: 2,  row: 1, lines: ['第4章は製造フェーズだ', '担当モジュールのコーディングをよろしく', 'スケジュールは…まあ予定通り進めば大丈夫', '設計の解釈は任せるよ、なんとかなるなる！', '佐藤くんがコードレビューしてくれるから、安心して書いていいよ', 'まあ90%まで行ったら、もう終わりも同然！（実際は違うけど）'] },
  { name: '佐藤先輩', col: 9, row: 3, lines: ['コード、書けてきた？', 'レビューでしっかり見るから安心してね', '表向きは『規約を守りましょう』なんだけど', '実際は『早く動くものを』が優先になりがち', '鈴木さんのとこ、「Aさんしか仕様を知らない」って言ってたでしょ。属人化って怖いんだよね', 'コードのコメント、ちゃんと書いておくと後で自分が救われるよ。経験談（笑）'] },
  { name: '鈴木さん', col: 14, row: 9, lines: ['うちの現場、テスト書かない文化なんですよ…', 'テストコード？工数に入ってないんで書きません（キリッ）', '『動けばOK』って空気、ちょっと怖いですよね', 'あと、このシステム、Aさんしか仕様分からないんです', '属人化ってやつ、目の前にあると笑えないですね', '佐藤さんが「鈴木さんのとこ大丈夫？」って気にかけてくれてるんですよね。ありがたいです', '田中さんの「なんとかなるなる」、最近は逆に笑えてきました。達観ってやつですかね（笑）'] },
];
...
const CHOICES: Record<'coding' | 'progress90', Choice[]> = {
  coding: [
    { text: '設計書を見ながら丁寧に実装', score: 10, result: '設計書通りに丁寧に実装。レビューでの指摘はほぼゼロで「バグの少ないコードだね」と褒められた。鈴木さんも「うちの現場でも見習ってほしい…」と羨ましそうに言っていた。[+10点]' },
    { text: 'とりあえず動けばOKで実装',     score: -5, result: 'とりあえず動くものは完成。だが2日後、想定外の入力でエラーが続出し、深夜まで原因調査することに。佐藤先輩に「ちゃんと設計書見ながら実装しようね」と言われた。[-5点]' },
    { text: '先輩のコードを参考にする',     score:  5, result: '先輩の過去のコードを参考に実装。「ちゃんと読んで理解してるね」と効率の良さを褒められた。[+5点]' },
    { text: '複雑な部分はTODOコメントだけ書いて後回しにする', score: 0, result: 'シンプルな部分から手をつけたのでペースは速かったが、結局複雑な部分を後回しにしただけで終盤に一気にしわ寄せが来た。「先に難しいところからやった方がいいかもね」と言われた。[±0点]' },
    { text: 'コピペで量産し、変数名は a, b, c のまま提出する', score: -10, result: '動くものはできたが、レビューで佐藤先輩が「これ…誰が読んでも分からないよ」と頭を抱えた。命名を全部やり直すことになり、結果的に倍以上の時間がかかった。[-10点]' },
  ],
  progress90: [
    { text: '正直に遅れを報告する',         score: 10, result: '「実は遅れています」と正直に報告。早めに人員調整が行われ、影響は最小限で済んだ。佐藤先輩が「ありがとう、早く言ってくれて」とホッとした顔をした。田中PMも「なんとかなったじゃん！」と呑気に言っていた。[+10点]' },
    { text: '「90%です」と言い続ける',      score: -5, result: '今週も「90%です」と報告。佐藤先輩の表情が一瞬曇った。いわゆる「90%シンドローム」が発動した。後で鈴木さんが「私も同じことやりました…連鎖しないといいですね」と小声で言ってくれた。[-5点]' },
...
const INCIDENT = {
  ...
  choices: [
    { text: '影響と工数を説明して交渉する',     score: 10, result: '現状のタスクと工数への影響を整理して佐藤先輩に共有。「それなら優先度を見直そう」と元請けとの交渉に持ち込んでもらえた。鈴木さんも「うちはいつも黙って引き受けちゃうんですよね…」と感心していた。[+10点]' },
    { text: '黙って引き受けて徹夜で対応する',   score: -5, result: '「やります…」と一人で抱え込み、土日返上で対応。月曜には動くものができたが、佐藤先輩に「先に相談してよ…」と心配された。田中PMは「なんとかなったじゃん！」と言っていたが、鈴木さんが「それを言うのは田中さんだけですよ…」と苦笑いしていた。[-5点]' },
...
export class Chapter4Scene extends Phaser.Scene {
  ...
  constructor() { super({ key: 'Chapter4Scene' }); }
  ...
  private buildHud() {
    ...
    this.hudTitle = this.add.text(10, 5, 'Day 7  製造フェーズ', { fontSize: '11px', color: '#7799aa', fontFamily: JP }).setScrollFactor(0).setResolution(2);
  ...
  private getLines(npc: NpcDef): string[] {
    if (npc.name === '田中PM') {
      ...
      if (this.gameStep === 1) return ['コーディング、進んでる？', '進捗会議で『順調です』って言えるくらいにね', '佐藤くんも気にかけてくれてるよ', '机で作業してね、応援してるよ'];
      return ['コーディングお疲れ様', '進捗90%でしょ？じゃあ残り10%もすぐだね', 'マイルストーンは守れそう？', '最後はみんな何とかするから、頼りにしてるよ', '佐藤くんも鈴木さんも、みんなで頑張ろう！'];
    }
    if (npc.name === '佐藤先輩') {
      if (this.gameStep < 2) return ['コード、書けてきた？', 'レビューでしっかり見るから安心してね', '表向きは『規約を守りましょう』なんだけど', '実際は『早く動くものを』が優先になりがち'];
      if (this.gameStep === 2) return ['進捗どう？実は90%って言ってたのに…', '先週も今週も90%って、ちょっと心配だよ', '鈴木さんも「うちも3週間続いた」って言ってたよ…あれが「90%シンドローム」ってやつ', '今の状況、正直に教えてくれる？'];
      return ['進捗報告ありがとう', '正直に言ってくれて、田中さんにもフォローしておいたよ', '鈴木さんも「正直に言うの、勇気いりますよね」って言ってたよ', '一緒に挽回しよう、一人で抱え込まないで'];
    }
    if (npc.name === '鈴木さん') {
  ...
  private getChoices(key: 'coding' | 'progress90' | 'incident'): Choice[] {
  ...
  private finishChoiceResult() {
    ...
    const next = missionKey === 'coding' ? 2 : 4;
  ...
  private showChapterClear() {
    saveChapterScore('chapter4', this.score);
    markChapterCleared('chapter4');
  ...
  update(_t: number, delta: number) {
    ...
    if (this.chapterClearShown) {
      if (Phaser.Input.Keyboard.JustDown(this.spaceKey) || this.virtualPad.isActionPressed()) {
        window.dispatchEvent(new CustomEvent('sier-chapter-cleared', { detail: { chapterId: 'chapter4' } }));
      }
      return;
    }
