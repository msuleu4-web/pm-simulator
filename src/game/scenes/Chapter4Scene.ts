...
const NPCS: NpcDef[] = [
  { name: '田中PM',   col: 2,  row: 3, lines: ['テストフェーズに入ります', 'テスト仕様書を作成してください'] },
  { name: '佐藤先輩', col: 10, row: 3, lines: ['テスト観点、ちゃんと洗い出せてる？', 'バグ票の書き方も大事だからね'] },
  { name: '鈴木さん', col: 18, row: 9, lines: ['テスト工数、いつも見積もりの半分しかないんですよ...', '結局残業でカバーすることになるんですよね...'] },
];

const CHOICES: Record<'testcase' | 'bugfix', Choice[]> = {
  testcase: [
    { text: '境界値分析も含めて網羅的に書く', score: 10, result: '高カバレッジのテストケース完成！[+10点]' },
    { text: '正常系だけテストする',           score: -5, result: '異常系でバグが見つかった...[-5点]' },
    { text: '先輩のテスト仕様書を参考にする', score:  5, result: '効率よく書けた[+5点]' },
  ],
  bugfix: [
    { text: '原因を特定してから修正する',     score: 10, result: '根本原因を解決！[+10点]' },
    { text: 'とりあえず動くように直す',       score: -5, result: '別のバグが発生した...[-5点]' },
    { text: '再現手順を確認してから対応',     score:  5, result: '正確に修正できた[+5点]' },
  ],
};
...
export class Chapter4Scene extends Phaser.Scene {
  ...
  private getLines(npc: NpcDef): string[] {
    if (npc.name === '田中PM') {
      if (this.gameStep === 0) return ['第4章はテストフェーズだ', 'テストフェーズに入ります。テスト仕様書を作成してください'];
      if (this.gameStep === 1) return ['テスト計画は順調？品質管理が肝心だよ'];
      return ['テスト進捗どう？品質をしっかり見ていこう'];
    }
    if (npc.name === '佐藤先輩') {
      if (this.gameStep < 2) return ['テスト観点、ちゃんと洗い出せてる？', 'バグ票の書き方も大事だからね'];
      if (this.gameStep === 2) return ['バグ票が3件上がってきたよ。対応お願い', '優先度の高いものから見てね'];
      return ['バグ修正お疲れ様。再テストもよろしくね'];
    }
    if (npc.name === '鈴木さん') {
      if (this.gameStep < 3) return ['テスト工数、いつも見積もりの半分しかないんですよ...', '結局残業でカバーすることになるんですよね...'];
      return ['バグ対応、お疲れ様です。うちもよくありますよ'];
    }
    return npc.lines;
  }
