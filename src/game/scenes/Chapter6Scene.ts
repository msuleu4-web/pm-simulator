...
const NPCS: NpcDef[] = [
  { name: '田中PM',   col: 2,  row: 1, lines: ['やあ…来てくれたか。第6章、炎上と立て直しのフェーズだ', 'リリース2週間前なのに、客先から新しい要望が来ててね…', 'まずそこのホワイトボード、見ておいてくれ', 'QCDの考え方、今からめちゃくちゃ大事になるから', '見たら、話しかけてくれ。状況を説明する', '佐藤くんが「田中さん、これ以上はチームが持ちません」ってちゃんと言ってくれてね。あれは、ありがたかった', '鈴木さんも踏ん張ってくれてる。みんなに頼りっぱなしで申し訳ない気持ちはあるんだよ…'] },
  { name: '佐藤先輩', col: 9, row: 3, lines: ['お疲れ様…ちょっと修羅場が続いてるんだ', '田中さんと一緒に、優先順位の整理をしてるところ', '新人くんも一緒に考えてくれると助かるよ', 'さっき田中さんに「このペースだとみんな壊れます」って言ったら、初めてちゃんと聞いてくれたよ', '鈴木さん、心折れそうって言ってたでしょ。あの人がそこまで言うって、相当きついんだよね'] },
  { name: '鈴木さん', col: 14, row: 9, lines: ['もう、なんでこのタイミングで仕様変更なんですか…', 'うちの会社、これ以上人出せないですよ…', '正直、心が折れそうです…', 'でも、ここでどう立て直すか、見ててくださいね', '佐藤さんが田中さんにちゃんと言ってくれたの、陰から見てたんです。かっこよかった…', 'こんな状況なのに、みんなが踏ん張ってくれてるから、私も頑張れる気がします'] },
];

const DOCUMENTS: ChapterDocument[] = [
  { id: 'doc-qcd', col: 17, row: 2, label: '資料📄',
...
const CHOICES: Record<'scopechange' | 'priority', Choice[]> = {
  scopechange: [
    { text: '影響調査→見積り→QCDへの影響を提示し、合意を取る', score: 10,
      result: '変更管理プロセスに沿って、影響範囲とスケジュール・コストへの影響を整理して提示。客先担当者は「次フェーズでの対応にしましょう」と納得してくれた。田中PM「これが本来のやり方だよ」。佐藤先輩も「よくやった」と後で静かに言ってくれた。[+10点]' },
    { text: '「やります」と即答して、徹夜で対応する', score: -5,
      result: '二つ返事で引き受けて、チーム総出で徹夜作業。なんとか形にはなったが、急ごしらえの実装はバグだらけで、結局リリース直前に大きな手戻りが発生した。鈴木さんが「私も何度かこれをやりました…」と遠い目をしていた。[-5点]' },
    { text: '「今は無理です」とだけ伝える', score: 5,
      result: 'きっぱり断ったことで一旦は収まったが、客先担当者は「融通が利かないなあ」と不満顔。佐藤先輩「断り方にも、もうちょっと工夫の余地があったかもね」。田中PMが後から代わりにフォローの電話をしていた。[+5点]' },
  ],
  priority: [
    { text: 'Must/Wantを仕分けし、優先度を客先と合意のうえ再調整する', score: 10,
      result: '必須機能と希望機能を切り分け、必須機能の品質を守りつつ、希望機能は次フェーズに。スケジュールも客先と合意の上で再設定。客先「誠実に説明してくれてありがとう」と納得。佐藤先輩が鈴木さんに「あの人、最近本当に頼もしくなったよ」と言っているのを後で聞いた。[+10点]' },
...
const INCIDENT = {
  ...
  choices: [
    { text: '影響範囲を全体に展開し、横展開で確認する', score: 10,
      result: '同じ実装パターンを使っている箇所を洗い出し、一斉に確認。他にも2箇所同様の問題が見つかり、リリース前に全て修正できた。田中PM「これが横展開ってやつか、助かったよ」。鈴木さんが「こういう対応ができる人、本当に信頼できます」と言ってくれた。[+10点]' },
    { text: '報告のあった画面だけ直してひとまず終わりにする', score: -5,
      result: '報告のあった箇所だけ直して安心していたら、翌日また別の画面で同じ不具合が発覚。「また出たんかい！」と田中PMが頭を抱えた。佐藤先輩が「同じパターンは横展開して確認しようね」と教えてくれた。[-5点]' },
    { text: '鈴木さんと一緒に他の画面も手分けして確認する', score: 5,
      result: '鈴木さんと手分けして確認。時間はかかったが、同様の問題を2箇所発見して対応できた。鈴木さん「一緒にやれてよかったです。あの状況で声かけてくれて、ありがたかった」。佐藤先輩も「チームワークだね」と笑顔だった。[+5点]' },
  ] as Choice[],
};
...
export class Chapter6Scene extends Phaser.Scene {
  ...
  private buildMap() {
    ...
    this.add.text(TILE + 4, TILE + 4, '対策本部', { fontSize: '10px', color: '#999', fontFamily: 'monospace' }).setResolution(2);
  ...
  private getLines(npc: NpcDef): string[] {
    if (npc.name === '田中PM') {
      ...
      return ['QCDの話、覚えてるかな？', '優先順位の整理、頼んだよ', '佐藤くんと鈴木さんにも状況は共有してあるから、一緒に動いてね', '…俺も、さすがに今回は「なんとかなる」とは言えないよ（苦笑）'];
    }
    if (npc.name === '佐藤先輩') {
      if (this.gameStep === 2) return npc.lines;
      if (this.gameStep === 3) return ['自分の机で、優先順位を整理してみてね', 'Must（必須）とWant（希望）を切り分けるのがポイントだよ', '鈴木さんも、状況の整理を手伝うって言ってくれてるよ', '田中さんにもちゃんと現状を伝えてあるから、信じて進めよう'];
      if (this.gameStep > 3) return ['ひと段落したね、お疲れ様！', 'こうやって一つひとつ立て直していくしかないからね', '鈴木さんも「なんとか乗り越えた…」って言ってたよ', '田中さんも今回ばかりはちょっと反省してたみたい（笑）'];
    }
    return npc.lines;
  }
  ...
  private finishChoiceResult() {
    ...
    const next = missionKey === 'scopechange' ? 2 : 4;
