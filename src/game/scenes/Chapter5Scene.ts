...
const NPCS: NpcDef[] = [
  { name: '田中PM',   col: 2,  row: 3, lines: ['いよいよリリースです', '本番環境へのデプロイ準備をしてください'] },
  { name: '佐藤先輩', col: 10, row: 3, lines: ['デプロイ手順、確認した？', '不安なところがあれば一緒に見るよ'] },
  { name: '鈴木さん', col: 18, row: 9, lines: ['運用保守って、終わりがないんですよ...', 'リリースしてもまた次の改修が始まりますからね'] },
];

const CHOICES: Record<'deploy' | 'incident', Choice[]> = {
  deploy: [
    { text: '手順書通りに慎重にリリース',         score: 10, result: '無事に本番稼働！[+10点]' },
    { text: '急いでそのままデプロイ',             score: -5, result: '本番障害発生...[-5点]' },
    { text: '先輩とダブルチェックしてリリース',   score:  5, result: '安心してリリースできた[+5点]' },
  ],
  incident: [
    { text: 'ログを確認して原因を特定',           score: 10, result: '迅速に復旧できた！[+10点]' },
    { text: 'とりあえずサーバー再起動',           score: -5, result: '一時的に直ったが再発...[-5点]' },
    { text: '影響範囲を確認してから対応',         score:  5, result: '冷静に対応できた[+5点]' },
  ],
};
...
export class Chapter5Scene extends Phaser.Scene {
  ...
  private getLines(npc: NpcDef): string[] {
    if (npc.name === '田中PM') {
      if (this.gameStep === 0) return ['いよいよ最終章だ', 'いよいよリリースです！本番環境へのデプロイ準備をしてください'];
      if (this.gameStep === 1) return ['リリース判定会議は通った。準備は万全かな'];
      return ['本番切替、緊張するね。落ち着いていこう'];
    }
    if (npc.name === '佐藤先輩') {
      if (this.gameStep < 2) return ['デプロイ手順、確認した？', '不安なところがあれば一緒に見るよ'];
      if (this.gameStep === 2) return ['本番から障害報告が来た！対応お願い', '切り戻すかどうかも判断してね'];
      return ['対応お疲れ様。よく頑張ったね'];
    }
    if (npc.name === '鈴木さん') {
      if (this.gameStep < 3) return ['運用保守って、終わりがないんですよ...', 'リリースしてもまた次の改修が始まりますからね'];
      return ['お疲れ様でした！', 'これで一区切りですね。本当によく頑張りました'];
    }
    return npc.lines;
  }
