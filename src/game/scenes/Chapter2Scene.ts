...
const NPCS: NpcDef[] = [
  { name: '田中PM',   col: 2,  row: 3, lines: ['基本設計書を作ってください', '要件定義書をベースに進めてね'] },
  { name: '佐藤先輩', col: 10, row: 3, lines: ['設計、順調？', 'わからないことがあったら聞いてね'] },
  { name: '鈴木さん', col: 18, row: 9, lines: ['元請けの設計書って、結構雑なんですよね...', 'うちはちゃんと書かないと、後で痛い目見ますから'] },
];

const CHOICES: Record<'design' | 'review', Choice[]> = {
  design: [
    { text: '要件をもう一度確認してから書く', score: 10, result: '正確な設計ができた！[+10点]' },
    { text: 'なんとなくで書き始める',           score: -5, result: '抜け漏れだらけ...[-5点]' },
    { text: '過去の設計書をコピーする',         score:  5, result: 'とりあえず形にはなった[+5点]' },
  ],
  review: [
    { text: '指摘を全部直して再レビュー依頼', score: 10, result: '品質が上がった！[+10点]' },
    { text: '1件だけ直して提出',               score: -5, result: 'また指摘が戻ってきた...[-5点]' },
    { text: '理由を聞いてから対応する',         score:  5, result: '理解が深まった[+5点]' },
  ],
};
...
export class Chapter2Scene extends Phaser.Scene {
  ...
  private getLines(npc: NpcDef): string[] {
    if (npc.name === '田中PM') {
      if (this.gameStep === 0) return ['第2章は基本設計フェーズだ', '基本設計書を作ってください。要件定義書をベースに頼むよ'];
      if (this.gameStep === 1) return ['設計書、進んでる？机で作業してね'];
      return ['設計書ありがとう。レビューに回しておくね'];
    }
    if (npc.name === '佐藤先輩') {
      if (this.gameStep < 2) return ['設計、順調？', '要件との整合性は特に注意してね'];
      if (this.gameStep === 2) return ['レビュー指摘が3件あるよ', '机に戻って対応しましょう'];
      return ['レビュー対応、お疲れ様！'];
    }
    if (npc.name === '鈴木さん') {
      if (this.gameStep < 3) return ['元請けの設計書って、結構雑なんですよね...', 'うちはちゃんと書かないと、後で痛い目見ますから'];
      return ['指摘対応、大変そうですね...頑張ってください'];
    }
    return npc.lines;
  }
  ...
  private showChapterClear() {
    ...
    this.clearScore.setText(`第2章「基本設計」クリア！\nスコア：${this.score}点`).setVisible(true);
