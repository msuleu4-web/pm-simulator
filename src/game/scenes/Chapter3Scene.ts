...
const NPCS: NpcDef[] = [
  { name: '田中PM',   col: 2,  row: 1, lines: ['第3章は基本設計フェーズだ', '要件定義書をベースに基本設計書を作ってくれ', 'テンプレ？前のプロジェクトのを流用していいよ', '品質と納期、両方守れるよね？(圧)', '細かい部分は現場のノリで察してくれ！', '細かいところは佐藤くんが補ってくれるから、大丈夫大丈夫'] },
  { name: '佐藤先輩', col: 9, row: 3, lines: ['設計、順調？要件との整合性に気をつけてね', '表向きは『テンプレ通りでOK』なんだけど', '実際はお客さんのこだわりが後から出てくるから', '行間を読む力が必要になるんだよね', '田中さんが「現場のノリで察して」って言うやつ、要注意ワードだから（笑）', '鈴木さんも「行間に何も書いてない」って言ってたでしょ。ほんと、あれはあるある'] },
  { name: '鈴木さん', col: 14, row: 9, lines: ['元請けの設計書って、結構雑なんですよね…', '『行間を読め』ってよく言われるんですけど', '行間に何も書いてないんですよ', 'うちは下請けなんで、ちゃんと書いとかないと痛い目見るんです', '佐藤さんが「行間を読む力が必要」って言ってましたけど、ほんとその通りですよね', '田中さんの「テンプレ流用でいいよ」、うちの現場でも聞いたことあります。で、流用元が古くて…（白目）'] },
];
...
const CHOICES: Record<'design' | 'review', Choice[]> = {
  design: [
    { text: '要件をもう一度確認してから書く', score: 10, result: '要件定義書を読み返してから書いたら抜け漏れのない設計書に。佐藤先輩に「直すところほぼないよ」と言われた。田中PMも「え、もう終わったの？さすが！」と驚いていた。[+10点]' },
  ...
  review: [
    { text: '指摘を全部直して再レビュー依頼', score: 10, result: '指摘事項をすべて反映して再レビュー依頼。佐藤先輩から「もう何も言うことないよ」と太鼓判をもらった。鈴木さんも「私のとこのメンバーにも見習ってほしい…」と羨ましそうだった。[+10点]' },
    { text: '1件だけ直して提出',               score: -5, result: '目立つ1件だけ直して提出したら「他の2件も直ってないよ？」とやんわり指摘され、気まずい空気に。鈴木さんが「うちでもよくあります…全部直してから出すの大事ですよ」と遠くから言ってくれた。[-5点]' },
...
const INCIDENT = {
  ...
  choices: [
    { text: '未確定部分を前提条件として明記し並行で進める', score: 10, result: '「ここはA案を前提に進めます」と一文添えておいたおかげで、後日仕様が変わっても直す範囲は最小限。佐藤先輩「その一文、神だね」。鈴木さんも「私もやっておけばよかった…（涙）」と言っていた。[+10点]' },
    { text: '何も考えず、今の仕様のまま全部作り切る',         score: -5, result: '案の定、翌週に仕様変更の連絡。作った分がまるごと手戻りに。田中PMは「そんなこともあるよねー」と他人事。佐藤先輩が「次は未確定部分を前提条件にして進めようね」とフォローしてくれた。[-5点]' },
    { text: '仕様が固まるまで、その部分は保留にする',       score:  5, result: '保留にした分、スケジュールはギリギリに。ただし手戻りはゼロで、佐藤先輩から「判断としては正しいよ」とフォローが入った。鈴木さんが「えらい、うちはいつも仕様未確定で走り始めちゃうんです」と感心していた。[+5点]' },
  ] as Choice[],
};
...
export class Chapter3Scene extends Phaser.Scene {
  ...
  private buildHud() {
    ...
    this.hudTitle = this.add.text(10, 5, 'Day 3  基本設計フェーズ', { fontSize: '11px', color: '#7799aa', fontFamily: JP }).setScrollFactor(0).setResolution(2);
  ...
  private getLines(npc: NpcDef): string[] {
    if (npc.name === '田中PM') {
      ...
      if (this.gameStep === 1) return ['設計書、進んでる？', '明日の朝イチで佐藤くんに見せられそう？', '机で作業してね、頑張って', '佐藤くん、朝は機嫌いいから早めに見せると良いよ（笑）'];
      return ['設計書ありがとう。レビューに回しておくね', '指摘？まあ、いつも通り少しはあるだろうけど', '気にせずサクッと直しちゃおう！', '佐藤くんが丁寧に見てくれるから安心して！'];
    }
    if (npc.name === '佐藤先輩') {
      if (this.gameStep < 2) return ['設計、順調？要件との整合性に気をつけてね', '表向きは『テンプレ通りでOK』なんだけど', '実際はお客さんのこだわりが後から出てくるから', '行間を読む力が必要になるんだよね'];
      if (this.gameStep === 2) return ['レビュー指摘が3件あるよ', '正直、田中さんが見落としてた所を拾った感じ', '鈴木さんも「あるある」って笑ってたよ', '机に戻って対応しましょう'];
      return ['レビュー対応、お疲れ様！', '指摘に向き合えるのは、いいエンジニアの第一歩', '田中さんも「成長してるじゃん」ってちょっと嬉しそうだったよ', '次の製造フェーズも一緒に頑張ろう'];
    }
    if (npc.name === '鈴木さん') {
      if (this.gameStep < 3) return ['元請けの設計書って、結構雑なんですよね…', '『行間を読め』ってよく言われるんですけど', '行間に何も書いてないんですよ', 'うちは下請けなんで、ちゃんと書いとかないと痛い目見るんです'];
      return ['指摘対応、大変そうですね…', 'うちでも真っ赤な設計書見て震えてますよ', '客先常駐だと、自社に帰れるの月1回ですね…', '佐藤さんに見てもらえてるなら大丈夫ですよ、あの人しっかりしてるから', '頑張ってください、応援してます'];
    }
    return npc.lines;
  }
  ...
  private finishChoiceResult() {
    ...
    const next = missionKey === 'design' ? 2 : 4;
