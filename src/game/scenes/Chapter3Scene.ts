...
const NPCS: NpcDef[] = [
  { name: '田中PM',   col: 2,  row: 3, lines: ['製造に入ってください', '担当モジュールのコーディングよろしく'] },
  { name: '佐藤先輩', col: 10, row: 3, lines: ['コード、書けてきた？', 'レビューでしっかり見るから安心してね'] },
  { name: '鈴木さん', col: 18, row: 9, lines: ['うちの現場、テスト書かない文化なんですよ...', '動けばOKって空気、ちょっと怖いですよね'] },
];

const CHOICES: Record<'coding' | 'progress90', Choice[]> = {
  coding: [
    { text: '設計書を見ながら丁寧に実装', score: 10, result: 'バグの少ないコードが完成！[+10点]' },
    { text: 'とりあえず動けばOKで実装',     score: -5, result: '後でバグだらけ...[-5点]' },
    { text: '先輩のコードを参考にする',     score:  5, result: '効率的に進められた[+5点]' },
  ],
  progress90: [
    { text: '正直に遅れを報告する',         score: 10, result: '早めの対策が打てた！[+10点]' },
    { text: '「90%です」と言い続ける',      score: -5, result: '90%シンドローム発動...[-5点]' },
    { text: '残作業を洗い出して報告',       score:  5, result: '具体的な計画が立てられた[+5点]' },
  ],
};
...
export class Chapter3Scene extends Phaser.Scene {
  ...
  private getLines(npc: NpcDef): string[] {
    if (npc.name === '田中PM') {
      if (this.gameStep === 0) return ['第3章は製造フェーズだ', '製造に入ってください。担当モジュールのコーディングよろしく'];
      if (this.gameStep === 1) return ['コーディング、進んでる？机で作業してね'];
      return ['コーディングお疲れ様。マイルストーンは守れそうかな？'];
    }
    if (npc.name === '佐藤先輩') {
      if (this.gameStep < 2) return ['コード、書けてきた？', 'レビューでしっかり見るから安心してね'];
      if (this.gameStep === 2) return ['進捗どう？実は90%って言ってたのに...', '今の状況、正直に教えてくれる？'];
      return ['進捗報告ありがとう。一緒に挽回しよう'];
    }
    if (npc.name === '鈴木さん') {
      if (this.gameStep < 3) return ['うちの現場、テスト書かない文化なんですよ...', '動けばOKって空気、ちょっと怖いですよね'];
      return ['進捗の話、他人事じゃないですね...うちもよくありますよ'];
    }
    return npc.lines;
  }
