...
const NPCS: NpcDef[] = [
  { name: '田中PM',   col: 2,  row: 1, lines: ['おお、配属初日だね！第1章は配属・キックオフだ', 'まずはそこの机にある資料、全部目を通しておいて', 'WBSと体制図、多重下請けの仕組み…全部大事だから', '読み終わったら、自己紹介がてら話しかけてね', '気合いと根性、あとは資料を読む力でなんとかなる！', '分からなかったら佐藤くんに聞いてね。あいつ、なんでも知ってるから'] },
  { name: '佐藤先輩', col: 9, row: 3, lines: ['はじめまして、よろしくね！', '来週から客先常駐が始まるから、心構えしておいてね', '現場には元請けさんも下請けさんもいて、最初は混乱すると思う', '分からないことがあったら、いつでも聞いてね', 'あ、あそこにいる鈴木さんって人、三次請けの方なんだけど、いろんな意味でリアルな話が聞けるよ', '田中さんの言う「なんとかなる」は…まあ、半分くらい信じておけば大丈夫（笑）'] },
  { name: '鈴木さん', col: 14, row: 9, lines: ['あ、新人さんですか？よろしくお願いします', 'うちは三次請けなので、現場では一番下っ端なんですよね…', '指示系統、最初はほんとうにややこしいので気をつけてくださいね', '元請けさんの指示と、うちの会社からの指示、両方聞く感じです', '佐藤さんから、新人さんが来るって聞いてました。いい人ですよ、佐藤さん', '田中さんも根はいい人ですけど、「なんとかなる」はだいたい「なんとかしてもらう」という意味なので（笑）'] },
];
...
const CHOICES: Record<'kickoff' | 'chain', Choice[]> = {
  kickoff: [
    { text: '資料を踏まえて、自分の担当範囲と疑問点を聞く', score: 10,
      result: '田中PMは「お、ちゃんと読んできたね！」と感心。WBSの読み方が分かっていると、初日からの印象が違う。佐藤先輩も「いいスタートだね」とにっこり。たまたま通りかかった鈴木さんも「最初からちゃんと確認できる人、貴重ですよ」と小声で言ってくれた。[+10点]' },
    { text: '分かったふりをして「大丈夫です」とだけ言う', score: -5,
      result: '「大丈夫です」と即答したが、後日、自分の担当範囲を勘違いしていたことが発覚。佐藤先輩に「最初に聞いておけばよかったのに…」と苦笑いされた。鈴木さんも「うちは聞けなくて痛い目見たことあります…」と遠い目をしていた。[-5点]' },
  ...
  chain: [
    { text: 'まず指示系統（誰の指示で動くか）を確認する', score: 10,
      result: '佐藤先輩はホッとした様子で「それ、最初に聞けるの偉いよ。新人がよく間違えるところなんだ」と教えてくれた。元請け・協力会社・客先常駐の関係がクリアになった。鈴木さんが後ろで「三次請けの自分でも、いまだに頭が混乱しますよ」とぽつりと言った。[+10点]' },
    { text: 'みんな同じチームだと思って、下請けの人に直接作業を頼む', score: -5,
      result: '鈴木さんに直接「これお願いします」と頼んだ瞬間、佐藤先輩が慌てて止めに入った。「それ、指揮命令系統が違うとマズいやつ…！偽装請負になっちゃう」と冷や汗をかきながら説明された。鈴木さんは苦笑いしながら「うちも、よく元請けさんから直接指示が来て困ってるんですよね」と言っていた。[-5点]' },
...
const INCIDENT = {
  ...
  choices: [
    { text: '体制図を確認し、呼び出し元が誰か確認してから向かう', score: 10,
      result: '体制図を見ると、呼び出し元は二次請けのリーダーだった。事前に「どの会議か」を確認してから向かったので、落ち着いて対応できた。佐藤先輩「準備してから動くの、大事だね」。田中PMも「おっ、さすがじゃん」とサムズアップ。[+10点]' },
    ...
    { text: '佐藤先輩に「これ、行った方がいいですか？」と確認する', score: 5,
      result: '佐藤先輩が一緒に確認してくれて、無事に正しい会議室にたどり着けた。「最初はみんなこうやって覚えるものだよ」と優しく言われた。後で田中PMに「佐藤くんに聞けたじゃん、正解！」と褒められた。[+5点]' },
  ] as Choice[],
};
...
export class Chapter1Scene extends Phaser.Scene {
  ...
  private getLines(npc: NpcDef): string[] {
    if (npc.name === '田中PM') {
      ...
      return ['さっき話したこと、覚えてる？', '資料、ちゃんと目を通しておいてね', '何か困ったことがあったら佐藤くんに聞いてね', '鈴木さんとも顔見知りになっておくと、現場で助かることあるよ'];
    }
    if (npc.name === '佐藤先輩') {
      if (this.gameStep === 2) return npc.lines;
      if (this.gameStep > 2) return ['客先常駐、緊張するけど一緒に頑張ろうね！', '分からないことがあったら、いつでも聞いてね', '鈴木さんとも話しておくといいよ。三次請けの視点、リアルで役に立つから'];
    }
    return npc.lines;
  }
