...
const NPCS: NpcDef[] = [
  { name: '韮沢CTO',   col: 2,  row: 1, lines: ['次の機能、設計書とかは要らないからそのまま作っちゃって', '動けばOKなんで、細かい仕様は実装しながら決めて', 'レビュー？うーん、そういうのは特にやってないかな', 'とりあえず触れるものを早く見たい！', 'なんかあったらDMで聞いて、よろしく〜'] },
  { name: '戸田さん', col: 9, row: 3, lines: ['さっきの韮沢さんの話、ちょっと心配になるよね', '私は自分用に「設計メモ」を残すようにしてるんだ', 'オダギリさんに見せると、結構喜ばれるよ', 'できたら、オダギリさんに見せるメモも、まとめてみてもらえる？'] },
  { name: 'オダギリさん', col: 14, row: 9, lines: ['お疲れ様です…', '正直、何が作られているのか、私もあまり把握できていなくて…', 'なので、簡単でいいので、設計の考え方が分かるメモがあると本当に助かります', '全部きっちり書く必要はないので、ポイントだけでも大丈夫です'] },
];

const DOCUMENTS: ChapterDocument[] = [
  { id: 'doc-vmodel', col: 12, row: 8, label: '資料📄',
    dialog: 'V字モデルの図が、どこかのプロジェクトの資料から印刷されたまま机に置かれている。\n\n「設計」→「設計レビュー」→「実装」→「テスト」の矢印が\nきれいに繋がっている図だ。\n\n「…Spireには、この『レビュー』の矢印が無いんだよな。\nでも、せめて自分の中だけでも、設計→確認の流れを意識してみよう。」',
    imageKey: 'v-model', imageLabel: 'V字モデル（参考資料）', required: true,
    blockedHint: '机の上の資料をチェックしてみよう…\n設計の進め方の参考になるかもしれない。' },
  { id: 'doc-qcd', col: 17, row: 8, label: '資料📄',
    dialog: 'QCD（品質・コスト・納期）について説明する資料が、本棚に挟まっていた。\n\n「動けばOK」で進めていると、特に"品質"の基準が\n曖昧になりがち――と書かれている。\n\n「オダギリさんが欲しがっている『最低限のドキュメント』って、\nこの"品質"を後から確認できるようにするためのものかもしれない。」',
    imageKey: 'qcd', required: true,
    blockedHint: '机の上の資料をチェックしてみよう…\n設計の進め方の参考になるかもしれない。' },
];

// スコアバランス: ミッション1(memo)+ミッション2(explain)+炎上イベント(INCIDENT)の
// 計3セット、各セット{+10,-5,+5}＋あるある失敗用の0/-10オプション。本章の最大30点。
// 全7章合計・難易度別最低点・エース判定の詳細は src/game/chapters.ts のコメント参照。
const CHOICES: Record<'memo' | 'explain', Choice[]> = {
  memo: [
    { text: 'なぜその設計にしたのか（理由・代替案・懸念点）を中心に、短くメモを残す', score: 10,
      result: '後で見返すと「なぜこうしたか」がすぐ分かるようになった。戸田さんに見せると「これ、まさに私がやってるやつだ」と笑っていた。[+10点]' },
    { text: '完成したコードを見れば分かるので、メモは特に残さない', score: -5,
      result: '数日後、なぜその実装にしたのか自分でも思い出せず、考え直す時間がかかってしまった。「動けばOK」のはずが、後から自分の時間を食ってしまった。[-5点]' },
    { text: '韮沢CTOのSlack発言をスクショして、メモに貼っておく', score: 5,
      result: '後から「あの時CTOがこう言ってたから、こうしました」と説明できるようになった。地味だが効果は大きかった。[+5点]' },
    { text: 'とりあえず「メモ」という名前のファイルだけ作って、後で書こうと思う', score: 0,
      result: '空のファイルだけが増えてしまった。「後で書く」はやはり来なかった。[±0点]' },
    { text: '誰も読まないし時間の無駄なので、メモ作成はスキップして次の作業に進む', score: -10,
      result: '後日、戸田さんに「あの設計、どういう考えだったの？」と聞かれて答えられず、気まずい空気になった。[-10点]' },
  ],
  explain: [
    { text: '専門用語を避けて、「何ができるようになるか」を中心に短くまとめる', score: 10,
      result: 'オダギリさんから「これ、すごく分かりやすいです！」と喜ばれた。韮沢CTOへの説明にもそのまま使えそうとのこと。[+10点]' },
    { text: '実装の詳細を、コードの構造そのままに細かく書く', score: -5,
      result: 'オダギリさんに見せたところ「えっと…ごめんなさい、ちょっと難しくて…」と困惑されてしまった。[-5点]' },
    { text: '戸田さんに「これ、オダギリさんに見せて大丈夫そうですか？」と一度確認してもらう', score: 5,
      result: '戸田さんが「うん、これなら大丈夫そう」とチェックしてくれた。安心して渡すことができた。[+5点]' },
    { text: '口頭で説明すればいいかと思い、メモは作らずそのまま話しかける', score: 0,
      result: '口頭だけでは後から見返せず、オダギリさんに「すみません、もう一度教えてもらえますか…」と聞かれてしまった。[±0点]' },
    { text: '「忙しいので、聞きたいことがあれば直接コードを見てください」と伝える', score: -10,
      result: 'オダギリさんは「あ…はい、すみません」と小さくなってしまった。その後、しばらく会話が減ってしまった気がする。[-10点]' },
  ],
};

// 炎上イベント — ミッションの合間に発生する「あるある」割り込みイベント
const INCIDENT = {
  notice: '🔥プチ炎上アラート🔥\nSlackに韮沢CTOから新しいDM。\n「やっぱりさっきの機能、ボタンの位置を逆にしたい！あと色も変えてもらえる？」',
  title: '🔥 動いているものに、いきなりの変更指示！どうする？',
  choices: [
    { text: '今の設計メモに「変更履歴」として追記しながら、変更内容を反映する', score: 10,
      result: 'メモに変更の経緯を残しておいたことで、後で「なぜ今の形になったか」が誰にでも分かる状態になった。戸田さんから「その積み重ね、地味だけど効くよ」と言われた。[+10点]' },
    { text: '何も記録せず、その場でコードだけ直して終わりにする', score: -5,
      result: '数日後、別の変更指示が来た際に「あれ、前はどうなってたんだっけ？」と分からなくなり、結局手戻りが発生した。[-5点]' },
    { text: '韮沢CTOに「了解です、ついでに他に変えたいところもあれば、まとめて教えてください」と聞く', score: 5,
      result: '韮沢CTOから「あ、それも助かる！」と他の変更点もまとめて教えてもらえた。一度で複数の変更に対応できた。[+5点]' },
    { text: '「はい、対応します」とだけ返信し、特に深く考えずに直す', score: 0,
      result: '直したこと自体は問題なかったが、特に記録も残らず、後から見返せる情報が増えなかった。[±0点]' },
    { text: '「さっき決めたのに、また変えるんですか…」と韮沢CTOに直接ぼやく', score: -10,
      result: '韮沢CTOは「あー、ごめんごめん」と軽く流したが、オダギリさんが心配そうに「大丈夫ですか…？」と声をかけてきた。[-10点]' },
  ] as Choice[],
};
...
export class HardChapter3Scene extends Phaser.Scene {
  ...
  private missionKey: 'memo' | 'explain' | 'incident' | null = null;
  ...
  constructor() { super({ key: 'HardChapter3Scene' }); }
  ...
  create() {
    ...
    this.showNotice('韮沢CTOから新しい依頼が来ています。\nまずは机の上の資料をチェックしてみましょう。', 4500);
  }
  ...
  private buildHud() {
    ...
    this.hudTitle = this.add.text(10, 5, '第3章　基本設計', { fontSize: '11px', color: '#7799aa', fontFamily: JP }).setScrollFactor(0).setResolution(2);
  ...
  private getLines(npc: NpcDef): string[] {
    if (npc.name === '韮沢CTO') {
      ...
      if (this.gameStep === 1) return ['例の機能、どんな感じ？', '別件もあるんだけど、それはまた後で', 'メモまとまったら教えてね'];
      return ['お、ありがとう！', 'こういうの残しといてくれると、後で自分も助かるかも', 'また何かあったら連絡するね'];
    }
    if (npc.name === '戸田さん') {
      if (this.gameStep >= 2 && this.gameStep < 3) return npc.lines;
      if (this.gameStep >= 3) return ['オダギリさんへの説明、ありがとう', 'こういうやり取り、地味だけど信頼に繋がるんだよね', '次の章もこの調子で頑張ろう'];
    }
    return npc.lines;
  }
  ...
  private getChoices(key: 'memo' | 'explain' | 'incident'): Choice[] {
    return key === 'incident' ? INCIDENT.choices : CHOICES[key];
  }

  private openChoices(key: 'memo' | 'explain' | 'incident') {
    this.missionKey = key;
    this.choiceState = 'open';
    const title = key === 'memo' ? '📝 自分用の設計メモを、どう書きますか？'
      : key === 'explain' ? '🗒️ オダギリさんに見せる説明メモを、どうまとめますか？'
      : INCIDENT.title;
  ...
  private showChapterClear() {
    saveChapterScore('hard-chapter3', this.score);
    markChapterCleared('hard-chapter3');
  ...
  update(_t: number, delta: number) {
    ...
    if (this.chapterClearShown) {
      if (Phaser.Input.Keyboard.JustDown(this.spaceKey) || this.virtualPad.isActionPressed()) {
        window.dispatchEvent(new CustomEvent('sier-chapter-cleared', { detail: { chapterId: 'hard-chapter3' } }));
      }
      return;
    }
    ...
    if (spaceJust && onDesk && missionActive) { this.openChoices(this.gameStep === 1 ? 'memo' : 'explain'); return; }
