...
const NPCS: NpcDef[] = [
  { name: '韮沢CTO',   col: 2,  row: 1, lines: ['おっ、新しい人？ Spireへようこそ。さっそくだけど、ウチはスピード重視でいくから', 'WBSとか体制図とか…そういうのは、正直作ってないんだよね。作る時間がもったいないから', 'とりあえず触ってみて。動かしながら覚えるのが一番早いから', '詳しいことは戸田さんとオダギリさんに聞いて。あとはSlackの過去ログ見て察して、的な', 'じゃ、よろしく！…あ、会議は基本やらないんで、何かあったらDMで'] },
  { name: '戸田さん', col: 9, row: 3, lines: ['はじめまして！Spireへようこそ。…資料が何も無くてびっくりした？', 'ここ、何もかも自分でルールを作るのが仕事みたいなところがあるんだよね', '韮沢さんは思いつきで喋るタイプだから、メモっておくと後で絶対役に立つよ', '困ったらいつでも聞いて。一緒に頑張ろう！'] },
  { name: 'オダギリさん', col: 14, row: 9, lines: ['あ、新しく入った方ですよね。お疲れ様です…', '私、Spire側の窓口的なことやってるんですけど、正直てんやわんやで…', 'せめて最低限の記録だけは残してもらえると、すごく助かります…', 'あと、韮沢が言ってたこと、忘れずにメモしておいてもらえると…本当に助かります'] },
];

const DOCUMENTS: ChapterDocument[] = [
  { id: 'doc-wbs', col: 7, row: 2, label: '資料📄',
    dialog: '机の上に1枚だけプリントされた紙が置いてある。\n\nタイトルは「WBSテンプレート」と書かれているが、項目はすべて空欄のままだ。\n\n「…テンプレートだけダウンロードして、中身は誰も書いていないのか。\nこれが「WBSが無い」ということか。」',
    imageKey: 'wbs', required: true,
    blockedHint: '机の上の資料をすべてチェックしてみよう…\nSpireでは自分から情報を取りにいく姿勢が大切だ。' },
  { id: 'doc-orgchart', col: 12, row: 2, label: '資料📄',
    dialog: 'ホワイトボードの脇に、図が一枚貼られている。\n\n「体制図（案）」と書かれているが、ほとんどの枠が「未定」「TBD」のままだ。\n\n「韮沢CTO、戸田さん、オダギリさんの名前だけは書いてある。\n…あとは全部「未定」か。これが今のSpireの体制、ということらしい。」',
    imageKey: 'org-chart', required: true,
    blockedHint: '机の上の資料をすべてチェックしてみよう…\nSpireでは自分から情報を取りにいく姿勢が大切だ。' },
  { id: 'doc-subcontract1', col: 17, row: 2, label: '資料📄',
    dialog: '引き出しの中に、見覚えのある図が入っていた。\n\n「SIer業界の多重下請け構造 ① ── 基本のしくみ」\n\n「自分の所属する協力会社は、ここでは戸田さんと同じ立ち位置みたいだ。\nSpireが発注元で、自分たちは現場に常駐している協力会社、ということか。」',
    imageKey: 'subcontract1', required: true,
    blockedHint: '机の上の資料をすべてチェックしてみよう…\nSpireでは自分から情報を取りにいく姿勢が大切だ。' },
];

// スコアバランス: ミッション1(assign)+ミッション2(rules)+炎上イベント(INCIDENT)の
// 計3セット、各セット{+10,-5,+5}＋あるある失敗用の0/-10オプション。本章の最大30点。
// 全7章合計・難易度別最低点・エース判定の詳細は src/game/chapters.ts のコメント参照。
const CHOICES: Record<'assign' | 'rules', Choice[]> = {
  assign: [
    { text: '「触ってみる前に、リポジトリと開発環境の構築手順だけ確認させてください」と聞く', score: 10,
      result: '韮沢CTOは「あ、それは戸田さんに聞いて。良い質問だね」と一言。最低限の環境情報を確認できたことで、後の作業がスムーズに進みそうだ。戸田さんも「最初にそれ聞けるの、地味に偉いよ」と頷いた。[+10点]' },
    { text: '「分かりました！」と即答し、何も確認せずにとにかく画面を触り始める', score: -5,
      result: '言われた通りすぐ触り始めたが、何のリポジトリかも分からず右往左往。半日溶かしてから、結局戸田さんに泣きついた。[-5点]' },
    { text: '戸田さんに「最初は何から見ればいいですか？」と聞く', score: 5,
      result: '戸田さんが「まずはSlackの#generalと#devを遡ってみて」と教えてくれた。一歩目の方向が分かり、ひとまず安心した。[+5点]' },
    { text: '「とりあえず触ってみて」を文字通り受け取り、適当に画面をクリックして回る', score: 0,
      result: '触ってみたものの、何が正解か分からないまま時間が過ぎていく。戸田さんに「まあ、最初はそんなもんだよ」と苦笑いされた。[±0点]' },
    { text: '「指示が曖昧すぎます」とその場で韮沢CTOに不満を言う', score: -10,
      result: '韮沢CTOは「えっ、そう？」と本気で不思議そうな顔をしている。気まずい空気の中、オダギリさんが慌ててフォローに入ってくれた。[-10点]' },
  ],
  rules: [
    { text: '「ルールが無いなら、自分用にメモやドキュメントを残していいですか？」と聞く', score: 10,
      result: '戸田さんは「それ、めちゃくちゃ大事！自分のためにも、後の人のためにもなるよ」と嬉しそうだった。記録を残す文化を、自分から作る一歩になった。[+10点]' },
    { text: '「ルールが無いなら何でも自由にやっていいですね」と好き勝手に進めてしまう', score: -5,
      result: '良かれと思って進めたが、後でオダギリさんに「それ、韮沢が考えてた方向とちょっと違ったみたいで…」と困り顔で言われてしまった。[-5点]' },
    { text: '「分からないことは、その都度オダギリさんに確認します」と伝える', score: 5,
      result: 'オダギリさんは「ぜひ、お願いします！」とほっとした様子だった。窓口を明確にしたことで、後のやり取りがスムーズになりそうだ。[+5点]' },
    { text: '戸田さんの話に「なるほど〜」と相槌だけして、特に何もしない', score: 0,
      result: '話は分かった気がしたが、特に行動には繋げなかった。後で「あの時の話、やっといてくれた？」と聞かれて気まずい思いをした。[±0点]' },
    { text: '「結局、何のルールもない会社なんですね」と戸田さんに言ってしまう', score: -10,
      result: '戸田さんの表情が一瞬曇った。「…まあ、そうなんだけどさ」と苦笑い。ベテランの戸田さんにも、思うところがあるのかもしれない。[-10点]' },
  ],
};

// 炎上イベント — ミッションの合間に発生する「あるある」割り込みイベント
const INCIDENT = {
  notice: '🔥プチ炎上アラート🔥\nSlackに韮沢CTOからの新着メッセージ。\n「さっき言ってた件、もう直した？できれば今すぐ確認したい」と一言だけ届いた！',
  title: '🔥 「さっき言ってた件」って、何のこと…？どうする？',
  choices: [
    { text: 'Slackの過去ログを遡り、韮沢CTOの直近の発言から該当箇所を推測してから返信する', score: 10,
      result: '数分前のCTOの発言に、それらしき話題を見つけた。「〇〇の件でしょうか？」と確認すると「そうそれ！」と返信が来た。記録を遡る癖が、早速役に立った。[+10点]' },
    { text: '「さっきの件、了解です！」とだけ返信し、見切り発車で適当な作業を進める', score: -5,
      result: '全く違う作業を進めてしまい、後で「それじゃないんだけど…」とCTOから一言。オダギリさんが間に入って事態を収拾してくれた。[-5点]' },
    { text: '戸田さんに「さっきの件って、何のことですかね…？」と相談する', score: 5,
      result: '戸田さんも「えーっと、たぶんあれかな」と一緒に推測してくれた。一人で抱え込まずに済んだ。[+5点]' },
    { text: '「すみません、「さっきの件」とは具体的にどれのことでしょうか？」とそのまま聞き返す', score: 0,
      result: '韮沢CTOからは「えーっと…なんだったかな、まあいいや、また連絡する」と返信が来た。間違いではないが、特に何も解決しなかった。[±0点]' },
    { text: '返信が来るまで、何もせずひたすら待つ', score: -10,
      result: '数十分後、オダギリさんから「韮沢さん、もう別の話で盛り上がってますよ…多分流れたやつですね」と苦笑い気味の報告が届いた。何も対応できなかった時間が、ただ過ぎてしまった。[-10点]' },
  ] as Choice[],
};
...
export class HardChapter1Scene extends Phaser.Scene {
  ...
  private missionKey: 'assign' | 'rules' | 'incident' | null = null;
  ...
  constructor() { super({ key: 'HardChapter1Scene' }); }
  ...
  create() {
    ...
    this.showNotice('本日からスタートアップ「Spire」に常駐配属されました。\nまずは机の上に積まれた資料に目を通してみましょう。', 4500);
  }
  ...
  private getLines(npc: NpcDef): string[] {
    if (npc.name === '韮沢CTO') {
      ...
      return ['お、進んでる？', 'なんかあったらSlackで聞いて', 'よろしくね〜'];
    }
    if (npc.name === '戸田さん') {
      if (this.gameStep === 2) return npc.lines;
      if (this.gameStep > 2) return ['ここの空気、ちょっとわかってきた？', '無理しすぎないでね、何かあったら声かけて'];
    }
    return npc.lines;
  }
  ...
  private getChoices(key: 'assign' | 'rules' | 'incident'): Choice[] {
    return key === 'incident' ? INCIDENT.choices : CHOICES[key];
  }

  private openChoices(key: 'assign' | 'rules' | 'incident') {
    this.missionKey = key;
    this.choiceState = 'open';
    const title = key === 'assign' ? '🤝 韮沢CTOの指示に、どう応じますか？'
      : key === 'rules' ? '🏢 Spireでの仕事の進め方について、どう動きますか？'
      : INCIDENT.title;
  ...
  private finishChoiceResult() {
    ...
    const next = missionKey === 'assign' ? 2 : 4;
  ...
  private showChapterClear() {
    saveChapterScore('hard-chapter1', this.score);
    markChapterCleared('hard-chapter1');
  ...
  update(_t: number, delta: number) {
    ...
    if (this.chapterClearShown) {
      if (Phaser.Input.Keyboard.JustDown(this.spaceKey) || this.virtualPad.isActionPressed()) {
        window.dispatchEvent(new CustomEvent('sier-chapter-cleared', { detail: { chapterId: 'hard-chapter1' } }));
      }
      return;
    }
