...
const CHOICES: Record<'m1' | 'm2', Choice[]> = {
  m1: [
    { text: 'ユーザーの動線を考えて画面遷移図から作る',     score: 10, result: '「論理的な設計、素晴らしいね」と山田課長も感心した様子。[+10点]' },
  ...
  m2: [
    { text: '3パターン用意して選んでもらう',               score: 10, result: '「比較できて選びやすいです！」と中村さんも喜んでくれた。[+10点]' },
    { text: '1パターンだけ見せて「これでいいですか？」',   score: -5, result: '「他の選択肢も見たかったです…」と中村さんに言われてしまった。[-5点]' },
    { text: 'プロトタイプを作って実際に触ってもらう',       score:  5, result: '「触れるのはわかりやすいです！」と好評だった。[+5点]' },
    { text: '良さそうな1パターンに絞って一気に作り込む',     score:  0, result: '完成度は高かったが、中村さんに「最初に方向性だけでも見せてもらえたら、もっと早く決められたかも」と言われた。[±0点]' },
  ],
};
...
const INCIDENT = {
  ...
  choices: [
    { text: '全体最適を説明して共通機能で対応提案する', score: 10, result: '「なるほど、それなら営業部も使いやすそうだ」と理解してもらえた。[+10点]' },
    { text: '言われるがまま専用ページを作る',           score: -5, result: '他の部署からも「うちにも専用ページを」と要望が相次ぎ、収拾がつかなくなった…[-5点]' },
    { text: '山田課長にエスカレーションする',           score:  5, result: '「了解、こっちで話しておくよ」と山田課長が引き受けてくれた。[+5点]' },
  ] as Choice[],
};
...
export class EasyChapter2Scene extends Phaser.Scene {
  ...
  private getLines(npc: NpcDef): string[] {
    if (npc.name === '山田課長') {
      if (this.gameStep === 0) return ['使いやすさが一番大事。社員全員が使うものだからね', 'ヒアリング結果をもとに、画面のたたき台を作ってみて', 'まずは木村くんに相談してみるといいよ'];
      if (this.gameStep < 3) return ['ワイヤーフレーム、進んでる？', '中村さんたちにも見てもらうといいね'];
      return ['いい画面になりそうだね', 'このまま進めよう'];
    }
    if (npc.name === '木村先輩') {
      if (this.gameStep === 0) return ['ワイヤーフレーム、作ってみる？', 'まずは画面遷移図から考えるといいよ', '机で考えてみて。できたら見せて'];
      if (this.gameStep < 3) return ['進んでる？', '画面遷移図、楽しみにしてるよ'];
      return ['ワイヤーフレーム見たよ、いいね', 'あとは中村さんたちにレビューしてもらおう'];
    }
    if (npc.name === '中村さん') {
      if (this.gameStep < 2) return ['新しい画面、楽しみにしてます！', 'わたしたち総務は、お知らせ機能をよく使うので', '見やすい画面になるといいなぁ'];
      if (this.gameStep === 2) return ['ワイヤーフレーム、見せてもらっていいですか？', 'お、いいですね！これなら使いやすそう', 'せっかくなので、みんなにも見てもらいましょう'];
      return ['レビューの件、ありがとうございました', '新しい画面、楽しみにしてます'];
    }
    return npc.lines;
  }
