...
const NPCS: NpcDef[] = [
  { name: '山田課長', col: 2, row: 1, lines: ['使いやすさが一番大事。社員全員が使うものだからね', 'ヒアリング結果をもとに、画面のたたき台を作ってみて', 'まぁ焦らずやろう。じっくり考えて大丈夫だよ'] },
  { name: '木村先輩', col: 9, row: 3, lines: ['ワイヤーフレーム、作ってみる？', 'まずは画面遷移図から考えるといいよ', '机で考えてみて。できたら見せて'] },
  { name: '中村さん', col: 6, row: 4, lines: ['新しい画面、楽しみにしてます！', 'わたしたち総務は、お知らせ機能をよく使うので', '見やすい画面になるといいなぁ'] },
];
...
const CHOICES: Record<'m1' | 'm2', Choice[]> = {
  m1: [
    { text: 'ユーザーの動線を考えて画面遷移図から作る',     score: 10, result: '「論理的な設計、素晴らしいね」と山田課長も感心した様子。隣で木村先輩も「前回のヒアリング、ちゃんと活きてるな」とうれしそう。[+10点]' },
    { text: 'いきなりデザインツールで見た目から作る',       score: -5, result: '「見た目はいいけど、導線がちょっとおかしいかも…」と木村先輩に指摘された。[-5点]' },
    { text: '既存ポータルの問題点を分析してから設計',       score:  5, result: '「現状分析から入るの、堅実だね」と木村先輩に言われた。[+5点]' },
  ],
  m2: [
    { text: '3パターン用意して選んでもらう',               score: 10, result: '「比較できて選びやすいです！」と中村さんも喜んでくれた。木村先輩も「準備の差が出るね」とニヤリ。[+10点]' },
    { text: '1パターンだけ見せて「これでいいですか？」',   score: -5, result: '「他の選択肢も見たかったです…」と中村さんに言われてしまった。[-5点]' },
    { text: 'プロトタイプを作って実際に触ってもらう',       score:  5, result: '「触れるのはわかりやすいです！」と中村さんに好評だった。[+5点]' },
    { text: '良さそうな1パターンに絞って一気に作り込む',     score:  0, result: '完成度は高かったが、中村さんに「最初に方向性だけでも見せてもらえたら、もっと早く決められたかも」と言われた。木村先輩も「次は早めに相談しよう」とフォロー。[±0点]' },
  ],
};

// 炎上イベント — ミッションの合間に発生する「あるある」割り込みイベント
const INCIDENT = {
  notice: '🔥プチ炎上アラート🔥\n営業部長が「うちの部署専用ページを作れ」と言い出した！',
  title: '🔥 営業部長からの要求、どう対応する？',
  choices: [
    { text: '全体最適を説明して共通機能で対応提案する', score: 10, result: '「なるほど、それなら営業部も使いやすそうだ」と理解してもらえた。木村先輩も「説明うまくなったな」と感心。[+10点]' },
    { text: '言われるがまま専用ページを作る',           score: -5, result: '他の部署からも「うちにも専用ページを」と要望が相次ぎ、収拾がつかなくなった…中村さんも「総務にまで問い合わせが来てます…」と困り顔。[-5点]' },
    { text: '山田課長にエスカレーションする',           score:  5, result: '「了解、こっちで話しておくよ」と山田課長が引き受けてくれた。木村先輩も「困ったときはそれでいいんだよ」とうなずいた。[+5点]' },
  ] as Choice[],
};

const MISSION_LABEL = [
  'NPCに話しかけよう',
  '📐 ワイヤーフレームを作成しよう（自分の机へ）',
  '📐 完了！ 中村さんに話しかけよう',
  '👀 ユーザーレビューを進めよう（自分の机へ）',
  '👀 全ミッション完了！',
];
...
export class EasyChapter2Scene extends Phaser.Scene {
  ...
  constructor() { super({ key: 'EasyChapter2Scene' }); }
  ...
  create() {
    ...
    this.showNotice('「ヒアリング結果がまとまった。次は画面のたたき台を作っていこう」', 4500);
  }
  ...
  private buildHud() {
    ...
    this.hudTitle = this.add.text(10, 5, 'Day 5  画面設計フェーズ', { fontSize: '11px', color: '#7799aa', fontFamily: JP }).setScrollFactor(0).setResolution(2);
  ...
  private getLines(npc: NpcDef): string[] {
    if (npc.name === '山田課長') {
      if (this.gameStep === 0) return ['お、もうだいぶ慣れてきたみたいだね', '使いやすさが一番大事。社員全員が使うものだからね', 'ヒアリング結果をもとに、画面のたたき台を作ってみて', 'まずは木村くんに相談してみるといいよ', 'そういえば今日は晴れて気持ちいいね。たまには換気もしないと'];
      if (this.gameStep < 3) return ['ワイヤーフレーム、進んでる？', '中村さんたちにも見てもらうといいね', '木村くんが頼りになるからどんどん聞いてね'];
      return ['いい画面になりそうだね', '木村くんと中村さんの連携もスムーズで助かるよ', 'このまま進めよう'];
    }
    if (npc.name === '木村先輩') {
      if (this.gameStep === 0) return ['お、来たね。前回のヒアリング、しっかり活かせてるみたいだね', 'ワイヤーフレーム、作ってみる？', 'まずは画面遷移図から考えるといいよ', 'さっき中村さんから電話あったよ。総務の画面のことで相談したいって', '机で考えてみて。できたら見せて'];
      if (this.gameStep < 3) return ['進んでる？', '画面遷移図、楽しみにしてるよ', 'そういえば中村さんがお菓子くれてさ。一緒にどう？'];
      return ['ワイヤーフレーム見たよ、いいね', '中村さんにも見せたら喜びそうだ', 'あとは中村さんたちにレビューしてもらおう'];
    }
    if (npc.name === '中村さん') {
      if (this.gameStep < 2) return ['あ、こんにちは。前回お話ししたこと、ちゃんと活かしてもらえて嬉しいです', '新しい画面、楽しみにしてます！', '木村さんからも、進んでるよって聞いてますよ', 'わたしたち総務は、お知らせ機能をよく使うので', '見やすい画面になるといいなぁ'];
      if (this.gameStep === 2) return ['ワイヤーフレーム、見せてもらっていいですか？', 'お、いいですね！これなら使いやすそう', '木村さんにも電話で少し聞いてたんですけど、想像より良くて驚きました', 'せっかくなので、みんなにも見てもらいましょう', 'お茶でもどうぞ。最近お気に入りのお菓子があるんです'];
      return ['レビューの件、ありがとうございました', '木村さんも「いい出来だ」って言ってましたよ', '新しい画面、楽しみにしてます'];
    }
    return npc.lines;
  }
  ...
  private openChoices(key: 'm1' | 'm2' | 'incident') {
    ...
    const title = key === 'm1' ? '📐 ワイヤーフレームをどう作成しますか？'
      : key === 'm2' ? '👀 ユーザーレビューをどう進めますか？'
      : INCIDENT.title;
  ...
  update(_t: number, delta: number) {
    ...
    if (this.chapterClearShown) {
      if (Phaser.Input.Keyboard.JustDown(this.spaceKey) || this.virtualPad.isActionPressed()) {
        window.dispatchEvent(new CustomEvent('sier-chapter-cleared', { detail: { chapterId: 'easy-chapter2' } }));
      }
      return;
    }
