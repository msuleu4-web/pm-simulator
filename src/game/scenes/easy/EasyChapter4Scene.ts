...
const CHOICES: Record<'m1' | 'm2', Choice[]> = {
  m1: [
    { text: 'テスト観点一覧を作って網羅的にテストケースを洗い出す', score: 10, result: '「これなら漏れがなさそうだね」と山田課長も安心した様子。[+10点]' },
    { text: '思いつくままに触ってバグを探す',                       score: -5, result: 'あとで見直すと、確認できていない画面がいくつも残っていた…[-5点]' },
  ...
  m2: [
    { text: '原因調査→修正→影響範囲確認→テストの手順で対応',       score: 10, result: '「丁寧な進め方、安心して任せられるね」と評価された。[+10点]' },
    { text: '該当箇所だけ直して再テストせず完了',                   score: -5, result: '別の画面で新しい不具合が見つかってしまった…[-5点]' },
...
const INCIDENT = {
  ...
  choices: [
    { text: '対象ブラウザを整理して、IE非対応の方針を提案する', score: 10, result: '「それなら現実的だね」と山田課長も納得。中村さんのPCだけ別途案内することに。[+10点]' },
    { text: 'IE11対応のためにCSSを全部書き直す',                 score: -5, result: '想定以上に時間がかかり、スケジュールが後ろ倒しに…[-5点]' },
    { text: '総務部にChrome導入を相談する',                       score:  5, result: '「総務部のPCにもChromeを入れてもらえました」と中村さんから報告。[+5点]' },
  ] as Choice[],
};
...
export class EasyChapter4Scene extends Phaser.Scene {
  ...
  private getLines(npc: NpcDef): string[] {
    if (npc.name === '山田課長') {
      if (this.gameStep === 0) return ['社内とはいえ品質は大事だよ', '全社員が使うものだから、バグがあると信頼を失ってしまうからね', 'まずはテスト計画から始めようか'];
      if (this.gameStep < 3) return ['テスト、順調そう？', '焦らず一つずつ確認していこう'];
      return ['バグ対応、お疲れさま', '品質、しっかり保てているね'];
    }
    if (npc.name === '木村先輩') {
      if (this.gameStep === 0) return ['テスト計画、立ててみようか', 'テスト観点表のフォーマット、共有するよ', '机で計画を立ててみて'];
      if (this.gameStep === 1) return ['計画、進んでる？', '網羅的にできそうかな'];
      return ['テスト計画、見たよ。いいね', 'バグが見つかったら、丁寧に対応していこう'];
    }
    if (npc.name === '中村さん') {
      if (this.gameStep < 2) return ['動作確認、お疲れさまです', '実は気になるところがあって…', 'テストが終わったら、聞いてもらえますか？'];
      if (this.gameStep === 2) return ['あの、ボタンを押しても反応しないページがあるんです', '私のパソコンだと、レイアウトも崩れて見えて…', '確認してもらえますか？'];
      return ['さっきの不具合、直してもらえたんですね', 'ありがとうございます、助かりました'];
    }
    return npc.lines;
  }
