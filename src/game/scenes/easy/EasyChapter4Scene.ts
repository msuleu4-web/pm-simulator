...
const NPCS: NpcDef[] = [
  { name: '山田課長', col: 2, row: 1, lines: ['社内とはいえ品質は大事だよ', '全社員が使うものだから、バグがあると信頼を失ってしまうからね', 'テスト計画、立ててみようか'] },
  { name: '木村先輩', col: 9, row: 3, lines: ['テスト、順調？', 'テスト観点表のフォーマット、共有するよ', '網羅的にやってみよう'] },
  { name: '中村さん', col: 6, row: 4, lines: ['動作確認、お疲れさまです', '実は気になるところがあって…', 'あとで聞いてもらえますか？'] },
];
...
const CHOICES: Record<'m1' | 'm2', Choice[]> = {
  m1: [
    { text: 'テスト観点一覧を作って網羅的にテストケースを洗い出す', score: 10, result: '「これなら漏れがなさそうだね」と山田課長も安心した様子。隣で木村先輩も「さすが、頼もしくなってきたな」とにっこり。[+10点]' },
    { text: '思いつくままに触ってバグを探す',                       score: -5, result: 'あとで見直すと、確認できていない画面がいくつも残っていた…木村先輩も「もったいない、観点表があれば防げたのに」と苦笑い。[-5点]' },
    { text: '主要な画面遷移だけ重点的にテスト',                     score:  5, result: '「まずは要所から、効率的だね」と木村先輩に言われた。[+5点]' },
    { text: '観点一覧は作ったが、時間切れで一部しか実行できない',   score:  0, result: '一覧自体は良い出来だったが、時間が足りず半分しか実行できなかった。「作るだけで終わらず、優先度をつけて実行する時間も確保しよう」とアドバイスされた。[±0点]' },
  ],
  m2: [
    { text: '原因調査→修正→影響範囲確認→テストの手順で対応',       score: 10, result: '「丁寧な進め方、安心して任せられるね」と評価された。中村さんも「直り具合、ちゃんと確認しますね」と画面を覗き込んでくれた。[+10点]' },
    { text: '該当箇所だけ直して再テストせず完了',                   score: -5, result: '別の画面で新しい不具合が見つかってしまった…中村さんに「あ、こっちもまだ崩れてます…」と申し訳なさそうに報告されてしまった。[-5点]' },
    { text: 'バグ管理票に記録してから修正に着手',                   score:  5, result: '「記録、大事だよね」と木村先輩にうなずかれた。[+5点]' },
  ],
};

// 炎上イベント — ミッションの合間に発生する「あるある」割り込みイベント
const INCIDENT = {
  notice: '🔥プチ炎上アラート🔥\nIE11でだけレイアウトが崩れることが判明（中村さんのPC）！',
  title: '🔥 IE11のレイアウト崩れ、どう対応する？',
  choices: [
    { text: '対象ブラウザを整理して、IE非対応の方針を提案する', score: 10, result: '「それなら現実的だね」と山田課長も納得。中村さんのPCだけ別途案内することに。木村先輩も「説明、一緒に行こうか」と気を遣ってくれた。[+10点]' },
    { text: 'IE11対応のためにCSSを全部書き直す',                 score: -5, result: '想定以上に時間がかかり、スケジュールが後ろ倒しに…中村さんも「無理させちゃってすみません」と恐縮していた。[-5点]' },
    { text: '総務部にChrome導入を相談する',                       score:  5, result: '「総務部のPCにもChromeを入れてもらえました」と中村さんから報告。「ついでにお菓子も差し入れしてくれましたよ」と木村先輩がニヤニヤしている。[+5点]' },
  ] as Choice[],
};
...
export class EasyChapter4Scene extends Phaser.Scene {
  ...
  constructor() { super({ key: 'EasyChapter4Scene' }); }
  ...
  create() {
    ...
    this.showNotice('「実装が一通り終わった。ここからはテスト・品質確認フェーズだ」', 4500);
  }
  ...
  private buildHud() {
    ...
    this.hudTitle = this.add.text(10, 5, 'Day 15  テストフェーズ', { fontSize: '11px', color: '#7799aa', fontFamily: JP }).setScrollFactor(0).setResolution(2);
  ...
  private getLines(npc: NpcDef): string[] {
    if (npc.name === '山田課長') {
      if (this.gameStep === 0) return ['社内とはいえ品質は大事だよ', '全社員が使うものだから、バグがあると信頼を失ってしまうからね', '木村くんから聞いたけど、最近いい感じに育ってるみたいだね', 'まずはテスト計画から始めようか'];
      if (this.gameStep < 3) return ['テスト、順調そう？', 'さっき木村くんと話してたんだけど、「あの新人、最近いいね」って', '焦らず一つずつ確認していこう'];
      return ['バグ対応、お疲れさま', '品質、しっかり保てているね', '中村さんからも「動きが速くて助かった」って聞いたよ'];
    }
    if (npc.name === '木村先輩') {
      if (this.gameStep === 0) return ['テスト計画、立ててみようか', 'テスト観点表のフォーマット、共有するよ', 'さっき中村さんとも話してたんだけど、テストには協力してくれるって', '机で計画を立ててみて'];
      if (this.gameStep === 1) return ['計画、進んでる？', '網羅的にできそうかな', 'そういえばお菓子もらったから、休憩のとき声かけて'];
      return ['テスト計画、見たよ。いいね', 'バグが見つかったら、丁寧に対応していこう', '中村さんもフィードバック楽しみにしてるみたいだよ'];
    }
    if (npc.name === '中村さん') {
      if (this.gameStep < 2) return ['動作確認、お疲れさまです', '木村さんから計画のこと聞きました。私もできることは協力しますね', '実は気になるところがあって…', 'テストが終わったら、聞いてもらえますか？'];
      if (this.gameStep === 2) return ['あの、ボタンを押しても反応しないページがあるんです', '私のパソコンだと、レイアウトも崩れて見えて…', '実際に使ってみて気づいたので、よかったら参考にしてください', '確認してもらえますか？'];
      return ['さっきの不具合、直してもらえたんですね', 'ありがとうございます、助かりました', '最近、お願いしたことちゃんと返ってくるので安心して頼めます', 'あ、今日は天気いいですね。お昼、外で食べようかな'];
    }
    return npc.lines;
  }
  ...
  private openChoices(key: 'm1' | 'm2' | 'incident') {
    ...
    const title = key === 'm1' ? '📋 テスト計画をどう立てますか？'
      : key === 'm2' ? '🐛 バグをどう修正しますか？'
      : INCIDENT.title;
  ...
  update(_t: number, delta: number) {
    ...
    if (this.chapterClearShown) {
      if (Phaser.Input.Keyboard.JustDown(this.spaceKey) || this.virtualPad.isActionPressed()) {
        window.dispatchEvent(new CustomEvent('sier-chapter-cleared', { detail: { chapterId: 'easy-chapter4' } }));
      }
      return;
    }
