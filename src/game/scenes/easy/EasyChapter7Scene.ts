...
const CHOICES: Record<'m1' | 'm2', Choice[]> = {
  m1: [
    { text: '事実をベースにKeep・Problem・Tryを整理する',     score: 10, result: '「客観的にまとめられてるね」と山田課長も感心した様子。[+10点]' },
    { text: '良かったことだけをまとめる',                       score: -5, result: '「課題も振り返らないと、次に繋がらないよ」と木村先輩に指摘された。[-5点]' },
    { text: '先輩に相談しながら整理する',                       score:  5, result: '「一緒に振り返ると、気づきも増えるね」と言われた。[+5点]' },
  ],
  m2: [
...
const INCIDENT = {
  ...
  title: '🔥 新人教育係、どう対応する？',
  choices: [
    { text: '自分が教わったことをもとに引継ぎ資料を作成する',       score: 10, result: '「自分の経験を整理するのは、いい振り返りにもなるね」と山田課長に言われた。[+10点]' },
    { text: '特に準備せず、その場で口頭で教える',                   score: -5, result: '新人さんから「何をすればいいかよくわからなくて…」と相談されてしまった。[-5点]' },
    { text: '木村先輩に教え方を相談してから準備する',               score:  5, result: '「先輩のやり方、参考になるね」と取り入れることができた。[+5点]' },
  ] as Choice[],
};
...
export class EasyChapter7Scene extends Phaser.Scene {
  ...
  private getLines(npc: NpcDef): string[] {
    if (npc.name === '山田課長') {
      if (this.gameStep === 0) return ['3ヶ月間、お疲れさまでした', 'このプロジェクトを振り返ってみようか', 'まずはKPTでまとめてみよう'];
      if (this.gameStep < 3) return ['振り返り、進んでる？', '良かったことも、課題も、どちらも大事だよ'];
      return ['次期企画の提案、お疲れさま', '成長したね。これからも頼りにしてるよ'];
    }
    if (npc.name === '木村先輩') {
      if (this.gameStep === 0) return ['振り返り、やってみる？', 'KPTって聞いたことある？', 'Keep・Problem・Tryでまとめてみて。机でやってみよう'];
      if (this.gameStep === 1) return ['振り返り、進んでる？', '事実をベースに考えてみるといいよ'];
      return ['振り返り、見たよ。いいまとめだね', '次は次期企画の提案も考えてみようか'];
    }
    if (npc.name === '中村さん') {
      if (this.gameStep < 2) return ['ポータル、本当に使いやすくなりました！', '次は何を作るんですか？', '楽しみにしてます！'];
      if (this.gameStep === 2) return ['もしよかったら、わたしたちの要望も聞いてもらえますか？', '実は他にもこんな機能があったら…と思うことがあって', '今度、聞いてもらえますか？'];
      return ['企画書、見せてもらいました！', 'わたしたちの声、ちゃんと反映されていて嬉しいです'];
    }
    return npc.lines;
  }
