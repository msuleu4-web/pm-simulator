...
const CHOICES: Record<'m1' | 'm2', Choice[]> = {
  m1: [
    { text: '画面キャプチャを多めに使って手順を説明する',     score: 10, result: '「これなら迷わず使えそうだね」と木村先輩も納得。[+10点]' },
    { text: '文字だけで詳しく説明する',                         score: -5, result: '「文字が多くて読むのが大変そう…」と中村さんに言われてしまった。[-5点]' },
    { text: '操作の様子を動画で撮ってチュートリアルにする',     score:  5, result: '「動画もわかりやすいね」と山田課長に褒められた。[+5点]' },
  ],
  m2: [
    { text: '実際に画面を操作しながらライブデモを行う',         score: 10, result: '「実際の動きが見られてよかったです！」と中村さんも好評。[+10点]' },
    { text: 'パワーポイントの説明だけで進める',                 score: -5, result: '「実際の画面が見たかったです…」と中村さんに言われてしまった。[-5点]' },
    { text: 'リニューアル前後の画面を比較しながら見せる',       score:  5, result: '「変化がよくわかりました！」と好評だった。[+5点]' },
    { text: '画面共有だけして、説明はチャットで済ませる',       score:  0, result: '「文字だけだとちょっと伝わりにくいかも」と中村さんに言われたが、画面自体は見てもらえた。[±0点]' },
  ],
};
...
const INCIDENT = {
  ...
  title: '🔥 前社長の写真、どう対応する？',
  choices: [
    { text: 'すぐに最新の写真へ差し替える（5分で対応）',           score: 10, result: '「さすが、早い対応だね」と山田課長も安心した様子。[+10点]' },
    { text: '「仕様です」と言って放置する',                         score: -5, result: 'お披露目会で社長に気づかれ、気まずい空気になった…[-5点]' },
    { text: '念のため確認・謝罪してから30分かけて再デプロイする',   score:  5, result: '「丁寧な確認、大事だよね」と木村先輩にうなずかれた。[+5点]' },
  ] as Choice[],
};
...
export class EasyChapter5Scene extends Phaser.Scene {
  ...
  private getLines(npc: NpcDef): string[] {
    if (npc.name === '山田課長') {
      if (this.gameStep === 0) return ['いよいよリリースだね', '社内お披露目会も予定しているから、楽しみにしてて', 'まずは使い方マニュアルを作ろうか'];
      if (this.gameStep < 3) return ['マニュアル、順調？', 'わかりやすく書けてるといいね'];
      return ['お披露目会、お疲れさま', 'いいプレゼンだったよ'];
    }
    if (npc.name === '木村先輩') {
      if (this.gameStep === 0) return ['マニュアル、作ってみる？', '使う人の立場で書くのがポイントだよ', '机で作ってみて'];
      if (this.gameStep === 1) return ['マニュアル、進んでる？', '画面の説明、わかりやすいといいね'];
      return ['マニュアル、見たよ。いいね', 'お披露目会の準備も進めていこう'];
    }
    if (npc.name === '中村さん') {
      if (this.gameStep < 2) return ['新しいポータル、リリースされるんですね！', 'お披露目会、楽しみにしてます', '総務部のみんなにも伝えておきますね'];
      if (this.gameStep === 2) return ['お披露目会、わたしたちも参加します！', 'どんな発表になるのか楽しみです', '準備、頑張ってくださいね'];
      return ['お披露目会、すごく分かりやすかったです！', '早速使ってみます！'];
    }
    return npc.lines;
  }
