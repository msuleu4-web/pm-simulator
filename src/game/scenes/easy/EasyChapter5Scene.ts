...
const NPCS: NpcDef[] = [
  { name: '山田課長', col: 2, row: 1, lines: ['いよいよリリースだね', '社内お披露目会も予定しているから、楽しみにしてて', 'まぁ焦らずやろう。準備、頼んだよ'] },
  { name: '木村先輩', col: 9, row: 3, lines: ['マニュアル、作ってみる？', '使う人の立場で書くのがポイントだよ', '机で作ってみて'] },
  { name: '中村さん', col: 6, row: 4, lines: ['新しいポータル、リリースされるんですね！', 'お披露目会、楽しみにしてます', '総務部のみんなにも伝えておきますね'] },
];

const CHOICES: Record<'m1' | 'm2', Choice[]> = {
  m1: [
    { text: '画面キャプチャを多めに使って手順を説明する',     score: 10, result: '「これなら迷わず使えそうだね」と木村先輩も納得。中村さんも「総務のみんなにも配れそうです！」と嬉しそう。[+10点]' },
    { text: '文字だけで詳しく説明する',                         score: -5, result: '「文字が多くて読むのが大変そう…」と中村さんに言われてしまった。木村先輩も「もう少し見やすくしようか」と苦笑い。[-5点]' },
    { text: '操作の様子を動画で撮ってチュートリアルにする',     score:  5, result: '「動画もわかりやすいね」と山田課長に褒められた。中村さんも「これなら迷わず操作できそうです」と笑顔。[+5点]' },
  ],
  m2: [
    { text: '実際に画面を操作しながらライブデモを行う',         score: 10, result: '「実際の動きが見られてよかったです！」と中村さんも好評。木村先輩も「練習の成果が出たな」とニヤリ。[+10点]' },
    { text: 'パワーポイントの説明だけで進める',                 score: -5, result: '「実際の画面が見たかったです…」と中村さんに言われてしまった。山田課長も「次は工夫しようか」と苦笑い。[-5点]' },
    { text: 'リニューアル前後の画面を比較しながら見せる',       score:  5, result: '「変化がよくわかりました！」と中村さんが好評。木村先輩も「リニューアル感、伝わったね」と満足そう。[+5点]' },
    { text: '画面共有だけして、説明はチャットで済ませる',       score:  0, result: '「文字だけだとちょっと伝わりにくいかも」と中村さんに言われたが、画面自体は見てもらえた。木村先輩が「次は一緒にやろう」とフォロー。[±0点]' },
  ],
};

const INCIDENT = {
  notice: '🔥プチ炎上アラート🔥\nトップページに前社長の写真がそのまま残っていることが判明！',
  title: '🔥 前社長の写真、どう対応する？',
  choices: [
    { text: 'すぐに最新の写真へ差し替える（5分で対応）',           score: 10, result: '「さすが、早い対応だね」と山田課長も安心した様子。中村さんも「気づいてくれてよかったです」とほっとした表情。[+10点]' },
    { text: '「仕様です」と言って放置する',                         score: -5, result: 'お披露目会で社長に気づかれ、気まずい空気になった…木村先輩も「うわ…」と顔を覆っていた。[-5点]' },
    { text: '念のため確認・謝罪してから30分かけて再デプロイする',   score:  5, result: '「丁寧な確認、大事だよね」と木村先輩にうなずかれた。中村さんも「ほっとしました」と笑顔。[+5点]' },
  ] as Choice[],
};

const MISSION_LABEL = [
  'NPCに話しかけよう',
  '📖 使い方マニュアルを作成しよう（自分の机へ）',
  '📖 完了！ 中村さんに話しかけよう',
  '🎤 お披露目会の準備をしよう（自分の机へ）',
  '🎤 全ミッション完了！',
];
...
export class EasyChapter5Scene extends Phaser.Scene {
  ...
  constructor() { super({ key: 'EasyChapter5Scene' }); }
  ...
  create() {
    ...
    this.showNotice('「いよいよリリース。社内お披露目会に向けて準備を進めよう」', 4500);
  }
  ...
  private buildHud() {
    ...
    this.hudTitle = this.add.text(10, 5, 'Day 20  リリース準備', { fontSize: '11px', color: '#7799aa', fontFamily: JP }).setScrollFactor(0).setResolution(2);
  ...
  private getLines(npc: NpcDef): string[] {
    if (npc.name === '山田課長') {
      if (this.gameStep === 0) return ['いよいよリリースだね', '社内お披露目会も予定しているから、楽しみにしてて', '木村くんも気合入ってたよ。みんなで盛り上げよう', 'まずは使い方マニュアルを作ろうか'];
      if (this.gameStep < 3) return ['マニュアル、順調？', 'わかりやすく書けてるといいね', '中村さん、お披露目会をすごく楽しみにしてるみたいだよ'];
      return ['お披露目会、お疲れさま', 'いいプレゼンだったよ', 'みんなで作り上げたポータル、これからもよろしく頼むよ'];
    }
    if (npc.name === '木村先輩') {
      if (this.gameStep === 0) return ['マニュアル、作ってみる？', '使う人の立場で書くのがポイントだよ', 'さっき中村さんが「楽しみです！」って張り切ってたよ', '机で作ってみて'];
      if (this.gameStep === 1) return ['マニュアル、進んでる？', '画面の説明、わかりやすいといいね', 'そういえば今日、中村さんからクッキーもらったんだ。一個あげるよ'];
      return ['マニュアル、見たよ。いいね', 'お披露目会の準備も進めていこう', '中村さん、緊張してるみたいだから声かけてあげて'];
    }
    if (npc.name === '中村さん') {
      if (this.gameStep < 2) return ['新しいポータル、リリースされるんですね！', 'お披露目会、楽しみです…でもちょっと緊張します', '総務部のみんなにも伝えておきますね', '木村さんが「大丈夫、いつも通りで」って励ましてくれました'];
      if (this.gameStep === 2) return ['お披露目会、わたしたちも参加します！', 'どんな発表になるのか楽しみです', '今日は天気もいいし、なんだかいい日になりそうですね', '準備、頑張ってくださいね'];
      return ['お披露目会、すごく分かりやすかったです！', '早速使ってみます！', '木村さんと一緒に総務のみんなにも紹介しますね', 'ここまで丁寧に教えてもらえて、本当によかったです'];
    }
    return npc.lines;
  }
  ...
  private openChoices(key: 'm1' | 'm2' | 'incident') {
    ...
    const title = key === 'm1' ? '📖 使い方マニュアルをどう作成しますか？'
      : key === 'm2' ? '🎤 お披露目会のプレゼンをどう進めますか？'
      : INCIDENT.title;
  ...
  update(_t: number, delta: number) {
    ...
    if (this.chapterClearShown) {
      if (Phaser.Input.Keyboard.JustDown(this.spaceKey) || this.virtualPad.isActionPressed()) {
        window.dispatchEvent(new CustomEvent('sier-chapter-cleared', { detail: { chapterId: 'easy-chapter5' } }));
      }
      return;
    }
