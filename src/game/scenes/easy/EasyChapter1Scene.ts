...
const CHOICES: Record<'m1' | 'm2', Choice[]> = {
  m1: [
    { text: '要望を整理して優先度をつけてまとめる',           score: 10, result: '「きれいにまとまってるね！次のMTGで使おう」と山田課長も嬉しそう。[+10点]' },
    ...
    { text: '広く浅く、早めに切り上げて他の部署にも回る',     score:  0, result: '話は広く聞けたが、中村さんに「もっと詳しく聞いてほしかったです…」と言われてしまった。次は一つずつ丁寧に聞こうと反省。[±0点]' },
  ],
  m2: [
    { text: '機能一覧と画面イメージを簡単に描いてみる',       score: 10, result: '「おぉ、ビジュアルがあると伝わりやすい！」と木村先輩も驚き。[+10点]' },
...
const INCIDENT = {
  ...
  choices: [
    { text: 'スコープ外として丁重にお断りする',   score: 10, result: '社長も納得。「フェーズ2で検討しよう」ということに。[+10点]' },
    { text: '「やります！」と即答する',           score: -5, result: '工数が膨らんで、他の機能の開発が遅れてしまった…[-5点]' },
    { text: '調査して実現可能性をレポートする',   score:  5, result: '現実的な提案として好評。「よく調べたね」と評価された。[+5点]' },
  ] as Choice[],
};
...
export class EasyChapter1Scene extends Phaser.Scene {
  ...
  private getLines(npc: NpcDef): string[] {
    if (npc.name === '山田課長') {
      if (this.gameStep === 0) return ['社内ポータルが古くて誰も使ってないんだよね', '各部署にヒアリングして、どんな機能がほしいか聞いてきて', 'まずは総務の中村さんに話を聞いてみようか', 'まぁ焦らずやろう。納期はけっこう余裕あるからね'];
      if (this.gameStep < 3) return ['ヒアリング、順調そう？', '中村さんの話、参考になったでしょ', '焦らずまとめてね'];
      return ['要件一覧、見せてもらったよ', 'いい感じにまとまってるね', 'このペースでいこう'];
    }
    if (npc.name === '中村さん') {
      if (this.gameStep === 0) return ['あ、新人さんですね。ポータルの件、聞いてます', '正直、今のポータルは使いづらくて…リンク切れも多いんです', '総務からのお知らせ、もっと見やすく載せてほしいです', '備品の申請も今は紙でやっていて、ちょっと大変で…', 'フォーマットが決まってなくて、みんな書き方が違うのも困りごとです'];
      return ['さっきの話、参考になったかな？', '新しいポータル、期待してますね'];
    }
    if (npc.name === '木村先輩') {
      if (this.gameStep < 2) return ['要件ヒアリング、進んでる？', 'わからなかったら何でも聞いてね', '中村さんの話、しっかりメモしておくといいよ'];
      if (this.gameStep === 2) return ['お、ヒアリング終わった？', 'じゃあ、聞いた内容を要件一覧にまとめてみようか', '机で整理してみて。完成したら見せてね'];
      return ['要件一覧、見せてもらったよ', 'いいまとめだね', '次は画面のデザインだね、また一緒に頑張ろう'];
    }
    return npc.lines;
  }
