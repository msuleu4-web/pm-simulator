...
const NPC_SPRITE_KEY: Record<string, string> = { '山田課長': 'npc-yamada', '木村先輩': 'npc-kimura', '中村さん': 'npc-nakamura' };

const NPCS: NpcDef[] = [
  { name: '山田課長', col: 2, row: 1, lines: ['社内ポータルが古くて誰も使ってないんだよね', '各部署にヒアリングして、どんな機能がほしいか聞いてきて', 'まずは総務の中村さんに話を聞いてみようか', 'まぁ焦らずやろう。納期はけっこう余裕あるからね'] },
  { name: '木村先輩', col: 9, row: 3, lines: ['要件ヒアリング、進んでる？', 'わからなかったら何でも聞いてね', '中村さんの話、しっかりメモしておくといいよ'] },
  { name: '中村さん', col: 6, row: 4, lines: ['あ、新人さんですね。ポータルの件、聞いてます', '正直、今のポータルは使いづらくて…リンク切れも多いんです', '総務からのお知らせ、もっと見やすく載せてほしいです', '備品の申請も今は紙でやっていて、ちょっと大変で…', 'フォーマットが決まってなくて、みんな書き方が違うのも困りごとです'] },
];

// スコアバランス: ミッション1(m1)+ミッション2(m2)+炎上イベント(INCIDENT)の
// 計3セット、各セット{+10,-5,+5}。本章の最大30点 / 最小-15点(NORMAL)。
// 全7章合計・難易度別最低点・エース判定の詳細は src/game/chapters.ts のコメント参照。
const CHOICES: Record<'m1' | 'm2', Choice[]> = {
  m1: [
    { text: '要望を整理して優先度をつけてまとめる',           score: 10, result: '「きれいにまとまってるね！次のMTGで使おう」と山田課長が満足そう。隣で木村先輩も「教えた甲斐があったな」とニヤリ。[+10点]' },
    { text: '聞いたことをそのままメモだけする',               score: -5, result: '「うーん、もう少し整理してくれる？」と木村先輩に苦笑いされてしまった。[-5点]' },
    { text: '木村先輩にヒアリング項目を相談してから行く',     score:  5, result: '「事前準備、いいね。効率よくできた」と木村先輩に褒められた。[+5点]' },
    { text: '広く浅く、早めに切り上げて他の部署にも回る',     score:  0, result: '話は広く聞けたが、中村さんに「もっと詳しく聞いてほしかったです…」と言われてしまった。それを聞いた木村先輩「次は一つずつでいいよ」とフォローしてくれた。[±0点]' },
  ],
  m2: [
    { text: '機能一覧と画面イメージを簡単に描いてみる',       score: 10, result: '「おぉ、ビジュアルがあると伝わりやすい！」と木村先輩も驚き。たまたま通りかかった中村さんも覗き込んで「これなら使いやすそう！」と笑顔に。[+10点]' },
    { text: '箇条書きだけ送る',                               score: -5, result: '「もう少し具体的だといいなぁ」と木村先輩から優しく指摘された。[-5点]' },
    { text: '類似サービスの事例を添えて提出',                 score:  5, result: '「参考資料つき、助かる」と木村先輩から感謝された。[+5点]' },
  ],
};

// 炎上イベント — ミッションの合間に発生する「あるある」割り込みイベント
const INCIDENT = {
  notice: '🔥プチ炎上アラート🔥\n社長が突然「AIチャットボット入れたい」と言い出した！',
  title: '🔥 社長からの急な要望、どう対応する？',
  choices: [
    { text: 'スコープ外として丁重にお断りする',   score: 10, result: '山田課長が「それは俺から社長に話しておくよ」とフォロー。木村先輩も「いい判断」とうなずいた。[+10点]' },
    { text: '「やります！」と即答する',           score: -5, result: '工数が膨らんで、他の機能の開発が遅れてしまった…木村先輩も「うーん、無理しなくていいのに」と心配顔。[-5点]' },
    { text: '調査して実現可能性をレポートする',   score:  5, result: '現実的な提案として好評。「よく調べたね」と山田課長に評価された。[+5点]' },
  ] as Choice[],
};
...
export class EasyChapter1Scene extends Phaser.Scene {
  ...
  private missionKey: 'm1' | 'm2' | 'incident' | null = null;
  ...
  constructor() { super({ key: 'EasyChapter1Scene' }); }
  ...
  preload() {
    ...
    this.load.spritesheet('npc-yamada', '/game-assets/Bob_idle_16x16.png', { frameWidth: 16, frameHeight: 32 });
    this.load.spritesheet('npc-kimura', '/game-assets/Alex_idle_16x16.png', { frameWidth: 16, frameHeight: 32 });
    this.load.spritesheet('npc-nakamura', '/game-assets/Amelia_idle_16x16.png', { frameWidth: 16, frameHeight: 32 });
  }
  ...
  create() {
    ...
    this.showNotice('「社内ポータル、リニューアルすることになってね」\nまずは各部署へのヒアリングから始めよう', 4500);
  }
  ...
  private buildHud() {
    ...
    this.hudTitle = this.add.text(10, 5, 'Day 1  ヒアリング開始', { fontSize: '11px', color: '#7799aa', fontFamily: JP }).setScrollFactor(0).setResolution(2);
  ...
  private getLines(npc: NpcDef): string[] {
    if (npc.name === '山田課長') {
      if (this.gameStep === 0) return ['お、新人さんだね。よろしく', '社内ポータルが古くて誰も使ってないんだよね', '各部署にヒアリングして、どんな機能がほしいか聞いてきて', '教育係は木村くんに頼んだから、わからないことは何でも聞いてね', 'まずは総務の中村さんに話を聞いてみようか。話しやすい人だから安心して', 'まぁ焦らずやろう。納期はけっこう余裕あるからね'];
      if (this.gameStep < 3) return ['ヒアリング、順調そう？', '中村さんの話、参考になったでしょ。あの人、的確だから', '木村くんも様子見てくれてるみたいだし、焦らずまとめてね'];
      return ['要件一覧、見せてもらったよ', 'いい感じにまとまってるね', '木村くんも「筋がいいですよ」って言ってたよ', 'このペースでいこう'];
    }
    if (npc.name === '中村さん') {
      if (this.gameStep === 0) return ['あ、新人さんですね。ポータルの件、聞いてます', '木村さんから「丁寧に教えてあげて」って言われてるので、何でも聞いてくださいね', '正直、今のポータルは使いづらくて…リンク切れも多いんです', '総務からのお知らせ、もっと見やすく載せてほしいです', '備品の申請も今は紙でやっていて、ちょっと大変で…', 'フォーマットが決まってなくて、みんな書き方が違うのも困りごとです', '…つい色々話しすぎちゃいました、すみません'];
      return ['さっきの話、参考になったかな？', '新人さんなのに、ちゃんとメモ取ってくれて嬉しかったです', '新しいポータル、期待してますね'];
    }
    if (npc.name === '木村先輩') {
      if (this.gameStep < 2) return ['お、ヒアリング行ってきた？ 中村さんとはもう長い付き合いでね', 'わからなかったら何でも聞いてね。俺も新人の頃は右も左もわからなかったし', '中村さんの話、しっかりメモしておくといいよ。あの人、的を射てるから'];
      if (this.gameStep === 2) return ['お、ヒアリング終わった？ お疲れさま', 'じゃあ、聞いた内容を要件一覧にまとめてみようか', '形式は気にしなくていいよ。まずは書き出してみることが大事', '机で整理してみて。完成したら見せてね'];
      return ['要件一覧、見せてもらったよ', 'いいまとめだね。中村さんの話、ちゃんと拾えてる', '山田課長も褒めてたよ', '次は画面のデザインだね、また一緒に頑張ろう'];
    }
    return npc.lines;
  }
  ...
  private closeDialog() {
    ...
    if (npc.name === '中村さん' && this.gameStep === 0) {
      this.gameStep = 1; this.updateHud();
      this.showNotice('ミッション受諾！\n📋 ヒアリング内容を整理してください\n自分の机へ行こう', 3000);
    } else if (npc.name === '木村先輩' && this.gameStep === 2) {
  ...
  private openChoices(key: 'm1' | 'm2' | 'incident') {
    this.missionKey = key;
    this.choiceState = 'open';
    const title = key === 'm1' ? '📋 ヒアリング内容をどうまとめますか？'
      : key === 'm2' ? '📝 要件一覧をどう作成しますか？'
      : INCIDENT.title;
  ...
  update(_t: number, delta: number) {
    ...
    if (this.chapterClearShown) {
      if (Phaser.Input.Keyboard.JustDown(this.spaceKey) || this.virtualPad.isActionPressed()) {
        window.dispatchEvent(new CustomEvent('sier-chapter-cleared', { detail: { chapterId: 'easy-chapter1' } }));
      }
      return;
    }
    ...
    if (spaceJust && onDesk && missionActive) { this.openChoices(this.gameStep === 1 ? 'm1' : 'm2'); return; }
