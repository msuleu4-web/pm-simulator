...
const NPCS: NpcDef[] = [
  { name: '山田課長', col: 2, row: 1, lines: ['実装、頑張ってね', '困ったことがあれば、木村くんに聞いてみて', '無理せず、自分のペースで進めてね'] },
  { name: '木村先輩', col: 9, row: 3, lines: ['フレームワークはNext.jsでいこう', '社内案件だし、新しい技術に挑戦できるチャンスだよ', '掲示板機能、一緒に作っていこう'] },
  { name: '中村さん', col: 6, row: 4, lines: ['コーディング中なんですね、お邪魔してすみません', '木村さん、さっきも机から離れてお菓子探してましたよ（笑）', '新しいポータル、楽しみにしてます！', 'がんばってくださいね'] },
];
...
const CHOICES: Record<'m1' | 'm2', Choice[]> = {
  m1: [
    { text: '設計書に忠実に、テストも書きながら実装',     score: 10, result: '「コードの品質が高いね」と木村先輩も感心した様子。通りかかった中村さんも「なんか難しそうですね…」と画面を覗き込んでいた。[+10点]' },
    { text: 'とりあえず動くものを最速で作る',             score: -5, result: '「バグが多くて手戻りが発生しちゃったね…」と木村先輩に指摘された。[-5点]' },
    { text: '木村先輩のコードを参考にしながら実装',       score:  5, result: '「先輩のノウハウ、しっかり吸収できたね」と褒められた。「俺のコード、結構クセあるでしょ」と笑っていた。[+5点]' },
    { text: 'とりあえず動くものを作り、テストは後回し',   score:  0, result: '実装は早く終わったが、後からテストを書く時間が必要になり、結局トータルの時間は変わらなかった。「最初から一緒に書いておくと安心だよ」と木村先輩に言われた。[±0点]' },
  ],
  m2: [
    { text: '指摘理由を理解して改善案も一緒に考える',     score: 10, result: '「成長が見えるね」と木村先輩も嬉しそう。「さすが、教えた甲斐があるな」とちょっと得意げだった。[+10点]' },
    { text: '言われた箇所だけ機械的に直す',               score: -5, result: '「また同じパターンのミスが出ちゃったね…」と木村先輩に苦笑いされた。[-5点]' },
    { text: 'なぜその書き方がダメなのか質問する',         score:  5, result: '「質問力、大事だよね」と木村先輩に言われた。隣のデスクの中村さんも「いい質問ですね」とにっこり。[+5点]' },
  ],
};

// 炎上イベント — ミッションの合間に発生する「あるある」割り込みイベント
const INCIDENT = {
  notice: '🔥プチ炎上アラート🔥\nWordPressからのデータ移行で文字化けが大量発生！',
  title: '🔥 文字化け、どう対応する？',
  choices: [
    { text: '文字コードを調査して変換スクリプトを作成する', score: 10, result: '「原因から潰せるの、頼もしいね」と木村先輩に評価された。中村さんも「直ったんですね、よかった！」とほっとした様子。[+10点]' },
    { text: '手作業で一つずつ直す',                         score: -5, result: '件数が多すぎて、作業がいつまでも終わらない…様子を見た中村さんが「お菓子、いります？」と差し入れに来てくれた。[-5点]' },
    { text: '木村先輩に相談する',                           score:  5, result: '「文字コードの変換、一緒にやろう」と助けてもらえた。「データ移行はあるあるだからね」と笑っていた。[+5点]' },
  ] as Choice[],
};
...
export class EasyChapter3Scene extends Phaser.Scene {
  ...
  constructor() { super({ key: 'EasyChapter3Scene' }); }
  ...
  create() {
    ...
    this.showNotice('「画面設計が決まった。いよいよ実装フェーズ、Next.jsで作っていこう」', 4500);
  }
  ...
  private buildHud() {
    ...
    this.hudTitle = this.add.text(10, 5, 'Day 10  開発フェーズ', { fontSize: '11px', color: '#7799aa', fontFamily: JP }).setScrollFactor(0).setResolution(2);
  ...
  private getLines(npc: NpcDef): string[] {
    if (npc.name === '山田課長') {
      if (this.gameStep === 0) return ['実装、頑張ってね', '掲示板機能、木村くんと一緒に作ってみて', 'わからないことがあれば遠慮なく聞いてね', 'さっき木村くんが中村さんのお菓子に手を伸ばしてたけど、見なかったことにしよう'];
      if (this.gameStep < 3) return ['実装、進んでる？', '焦らずやってね', '今日は天気もいいし、たまには窓開けて休憩するのもいいよ'];
      return ['レビュー対応、お疲れさま', 'いいコードになってきたね', '木村くんも中村さんも褒めてたよ、いいチームだね'];
    }
    if (npc.name === '木村先輩') {
      if (this.gameStep === 0) return ['フレームワークはNext.jsでいこう', '社内案件だし、新しい技術に挑戦できるチャンスだよ', '掲示板機能、一緒に作っていこう', 'さっき中村さんに「またお菓子つまみに来たんですか」って怒られちゃってさ'];
      if (this.gameStep === 1) return ['実装、進んでる？', '困ったらいつでも声かけて', 'そろそろ昼だけど、今日のランチどうする？'];
      if (this.gameStep === 2) return ['コード見たよ、いくつか指摘があるから直してみようか', '机で確認してみて', '中村さんにも「新人さん、筋がいいですね」って言われてたよ'];
      return ['レビュー対応、いいね', '次のテストフェーズも一緒にがんばろう', '今度、中村さんも交えて三人でランチでもどう？'];
    }
    if (npc.name === '中村さん') return npc.lines;
    return npc.lines;
  }
  ...
  private closeDialog() {
    ...
    if (npc.name === '山田課長' && this.gameStep === 0) {
  ...
  private openChoices(key: 'm1' | 'm2' | 'incident') {
    ...
    const title = key === 'm1' ? '💻 掲示板機能をどう実装しますか？'
      : key === 'm2' ? '🔍 コードレビュー対応をどう進めますか？'
      : INCIDENT.title;
  ...
  update(_t: number, delta: number) {
    ...
    if (this.chapterClearShown) {
      if (Phaser.Input.Keyboard.JustDown(this.spaceKey) || this.virtualPad.isActionPressed()) {
        window.dispatchEvent(new CustomEvent('sier-chapter-cleared', { detail: { chapterId: 'easy-chapter3' } }));
      }
      return;
    }
