...
const NPCS: NpcDef[] = [
  ...
  { name: '中村さん', col: 6, row: 4, lines: ['コーディング中なんですね、お邪魔してすみません', '新しいポータル、楽しみにしてます！', 'がんばってくださいね'] },
];
...
const CHOICES: Record<'m1' | 'm2', Choice[]> = {
  m1: [
    { text: '設計書に忠実に、テストも書きながら実装',     score: 10, result: '「コードの品質が高いね」と木村先輩も感心した様子。[+10点]' },
    { text: 'とりあえず動くものを最速で作る',             score: -5, result: '「バグが多くて手戻りが発生しちゃったね…」と木村先輩に指摘された。[-5点]' },
    { text: '木村先輩のコードを参考にしながら実装',       score:  5, result: '「先輩のノウハウ、しっかり吸収できたね」と褒められた。[+5点]' },
    { text: 'とりあえず動くものを作り、テストは後回し',   score:  0, result: '実装は早く終わったが、後からテストを書く時間が必要になり、結局トータルの時間は変わらなかった。「最初から一緒に書いておくと安心だよ」と木村先輩に言われた。[±0点]' },
  ],
  m2: [
    { text: '指摘理由を理解して改善案も一緒に考える',     score: 10, result: '「成長が見えるね」と木村先輩も嬉しそう。[+10点]' },
    { text: '言われた箇所だけ機械的に直す',               score: -5, result: '「また同じパターンのミスが出ちゃったね…」と木村先輩に苦笑いされた。[-5点]' },
    { text: 'なぜその書き方がダメなのか質問する',         score:  5, result: '「質問力、大事だよね」と木村先輩に言われた。[+5点]' },
  ],
};
...
const INCIDENT = {
  ...
  title: '🔥 文字化け、どう対応する？',
  choices: [
    { text: '文字コードを調査して変換スクリプトを作成する', score: 10, result: '「原因から潰せるの、頼もしいね」と木村先輩に評価された。[+10点]' },
    { text: '手作業で一つずつ直す',                         score: -5, result: '件数が多すぎて、作業がいつまでも終わらない…[-5点]' },
    { text: '木村先輩に相談する',                           score:  5, result: '「文字コードの変換、一緒にやろう」と助けてもらえた。[+5点]' },
  ] as Choice[],
};
...
export class EasyChapter3Scene extends Phaser.Scene {
  ...
  private getLines(npc: NpcDef): string[] {
    if (npc.name === '山田課長') {
      if (this.gameStep === 0) return ['実装、頑張ってね', '掲示板機能、木村くんと一緒に作ってみて', 'わからないことがあれば遠慮なく聞いてね'];
      if (this.gameStep < 3) return ['実装、進んでる？', '焦らずやってね'];
      return ['レビュー対応、お疲れさま', 'いいコードになってきたね'];
    }
    if (npc.name === '木村先輩') {
      if (this.gameStep === 0) return ['フレームワークはNext.jsでいこう', '社内案件だし、新しい技術に挑戦できるチャンスだよ', '掲示板機能、一緒に作っていこう'];
      if (this.gameStep === 1) return ['実装、進んでる？', '困ったらいつでも声かけて'];
      if (this.gameStep === 2) return ['コード見たよ、いくつか指摘があるから直してみようか', '机で確認してみて'];
      return ['レビュー対応、いいね', '次のテストフェーズも一緒にがんばろう'];
    }
