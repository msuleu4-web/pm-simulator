...
const CHOICES: Record<'m1' | 'm2', Choice[]> = {
  m1: [
    { text: 'アクセスログとアンケート結果の両方を確認する',     score: 10, result: '「両方見れば、実態と感想のズレもわかるね」と木村先輩も感心した様子。[+10点]' },
    { text: '自分の感覚や印象だけで判断する',                     score: -5, result: '後で「データと全然違う結果だったね…」と指摘されてしまった。[-5点]' },
    { text: 'よく使われているページだけ確認する',                 score:  5, result: '「まずは人気ページから、効率的だね」と言われた。[+5点]' },
    { text: 'アクセスログだけサクッと確認する',                   score:  0, result: '大まかな傾向はつかめたが、利用者の感想までは分からなかった。「両方見るとより確実だね」とアドバイスされた。[±0点]' },
  ],
  m2: [
    { text: '検索キーワードに対応しつつ使い方ガイドも作成する',   score: 10, result: '「これなら検索もわかりやすさも両方カバーできるね」と山田課長も納得。[+10点]' },
    { text: '特に何もせず様子を見る',                             score: -5, result: '利用率は変わらず、中村さんから「やっぱり検索しづらいです…」と再度声が上がった。[-5点]' },
    { text: 'FAQページを新設する',                                 score:  5, result: '「よくある質問がまとまってて助かります！」と好評だった。[+5点]' },
  ],
};
...
const INCIDENT = {
  ...
  choices: [
    { text: '検索ログを調査して、よく検索されるキーワードを把握する', score: 10, result: '「原因から見ていくの、頼もしいね」と木村先輩に評価された。[+10点]' },
    { text: '検索ボタンを大きくするだけで対応する',                     score: -5, result: '見た目は変わったが、不満の声は変わらなかった…[-5点]' },
    { text: '中村さんに普段どんな言葉で検索しているか聞く',             score:  5, result: '「なるほど、そういう探し方をするんですね」と気づきがあった。[+5点]' },
  ] as Choice[],
};
...
export class EasyChapter6Scene extends Phaser.Scene {
  ...
  private getLines(npc: NpcDef): string[] {
    if (npc.name === '山田課長') {
      if (this.gameStep === 0) return ['ポータル、無事リリースできたね', 'これからは使ってもらいながら、改善していくフェーズだよ', 'まずは利用状況を分析してみようか'];
      if (this.gameStep < 3) return ['分析、進んでる？', '焦らずじっくりでいいよ'];
      return ['利用率向上の施策、お疲れさま', 'いい改善だったね'];
    }
    if (npc.name === '木村先輩') {
      if (this.gameStep === 0) return ['利用状況のログ、見てみる？', 'アンケートの結果も集まってきたよ', '分析、机でやってみて'];
      if (this.gameStep === 1) return ['分析、進んでる？', '気づいたことあった？'];
      return ['分析、見たよ。いいね', '次は利用率向上の施策を考えていこう'];
    }
    if (npc.name === '中村さん') {
      if (this.gameStep < 2) return ['新しいポータル、使ってます！', 'みんなにも聞いてみますね', '使いやすくなるといいなぁ'];
      if (this.gameStep === 2) return ['総務部のみんなにも聞いてみました', 'お知らせ機能は好評みたいです', 'でも検索機能については…ちょっと声がありました'];
      return ['新しい機能、早速使ってみました！', '前よりずっと使いやすくなりました'];
    }
    return npc.lines;
  }
