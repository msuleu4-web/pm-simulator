...
const NPCS: NpcDef[] = [
  { name: '山田課長', col: 2, row: 1, lines: ['ポータル、無事リリースできたね', 'これからは使ってもらいながら、改善していくフェーズだよ', 'まぁ焦らずやろう。じっくり育てていこう'] },
  { name: '木村先輩', col: 9, row: 3, lines: ['利用状況のログ、見てみる？', 'アンケートの結果も集まってきたよ', '分析、机でやってみて'] },
  { name: '中村さん', col: 6, row: 4, lines: ['新しいポータル、使ってます！', 'みんなにも聞いてみますね', '使いやすくなるといいなぁ'] },
];

const CHOICES: Record<'m1' | 'm2', Choice[]> = {
  m1: [
    { text: 'アクセスログとアンケート結果の両方を確認する',     score: 10, result: '「両方見れば、実態と感想のズレもわかるね」と木村先輩も感心した様子。隣で中村さんも「私のアンケート、役に立ちました？」と嬉しそうに聞いてきた。[+10点]' },
    { text: '自分の感覚や印象だけで判断する',                     score: -5, result: '後で「データと全然違う結果だったね…」と木村先輩に指摘されてしまった。中村さんも「せっかく集めたアンケート、見てもらえなくて残念です…」と少ししょんぼり。[-5点]' },
    { text: 'よく使われているページだけ確認する',                 score:  5, result: '「まずは人気ページから、効率的だね」と木村先輩。中村さんも「それなら私のアンケートも、後で見てもらえますか？」とのこと。[+5点]' },
    { text: 'アクセスログだけサクッと確認する',                   score:  0, result: '大まかな傾向はつかめたが、利用者の感想までは分からなかった。「両方見るとより確実だね」と木村先輩からアドバイスされ、中村さんも「アンケートもぜひ見てくださいね」と笑った。[±0点]' },
  ],
  m2: [
    { text: '検索キーワードに対応しつつ使い方ガイドも作成する',   score: 10, result: '「これなら検索もわかりやすさも両方カバーできるね」と山田課長も納得。木村先輩も「俺もそう思ってた」とニヤリ、中村さんは「総務のみんなにも教えますね！」と嬉しそう。[+10点]' },
    { text: '特に何もせず様子を見る',                             score: -5, result: '利用率は変わらず、中村さんから「やっぱり検索しづらいです…」と再度声が上がった。木村先輩も「うーん、これは早めに手を打った方がいいかもな」と腕を組んだ。[-5点]' },
    { text: 'FAQページを新設する',                                 score:  5, result: '「よくある質問がまとまってて助かります！」と中村さんが好評。木村先輩も「いいね、検索の方は次の課題かな」とコメントした。[+5点]' },
  ],
};

const INCIDENT = {
  notice: '🔥プチ炎上アラート🔥\n「検索しても欲しい情報が出てこない」と複数の部署から不満の声が！',
  title: '🔥 検索機能への不満、どう対応する？',
  choices: [
    { text: '検索ログを調査して、よく検索されるキーワードを把握する', score: 10, result: '「原因から見ていくの、頼もしいね」と木村先輩に評価された。中村さんも「さすがです！」と隣でうんうん頷いていた。[+10点]' },
    { text: '検索ボタンを大きくするだけで対応する',                     score: -5, result: '見た目は変わったが、不満の声は変わらなかった…木村先輩も「うーん、ちょっと的外れだったかもな」と苦笑い。[-5点]' },
    { text: '中村さんに普段どんな言葉で検索しているか聞く',             score:  5, result: '「なるほど、そういう探し方をするんですね」と気づきがあった。木村先輩も「現場の声を聞くの、大事だよな」と感心していた。[+5点]' },
  ] as Choice[],
};
...
export class EasyChapter6Scene extends Phaser.Scene {
  ...
  constructor() { super({ key: 'EasyChapter6Scene' }); }
  ...
  create() {
    ...
    this.showNotice('「ポータルをリリースして1週間。みんなの使い心地、聞いてみよう」', 4500);
  }
  ...
  private buildHud() {
    ...
    this.hudTitle = this.add.text(10, 5, 'Day 30  運用フィードバック', { fontSize: '11px', color: '#7799aa', fontFamily: JP }).setScrollFactor(0).setResolution(2);
  ...
  private getLines(npc: NpcDef): string[] {
    if (npc.name === '山田課長') {
      if (this.gameStep === 0) return ['ポータル、無事リリースできたね', 'これからは使ってもらいながら、改善していくフェーズだよ', '木村くんが利用ログをまとめてくれてるみたいだから、聞いてみて', '今日はちょっと曇りだけど、心は晴れやかだよ', 'まずは利用状況を分析してみようか'];
      if (this.gameStep < 3) return ['分析、進んでる？', '木村くんと中村さんも、いろいろ意見出してくれてるみたいだね', '焦らずじっくりでいいよ'];
      return ['利用率向上の施策、お疲れさま', '中村さんからも「使いやすくなった」って声が届いてるよ', 'いい改善だったね'];
    }
    if (npc.name === '木村先輩') {
      if (this.gameStep === 0) return ['利用状況のログ、見てみる？', 'アンケートの結果も集まってきたよ', '中村さんが総務のみんなにも聞いて回ってくれたんだ', 'もうほぼ同僚みたいなもんだし、率直に話そう', '分析、机でやってみて'];
      if (this.gameStep === 1) return ['分析、進んでる？', '気づいたことあった？', '俺もさっき中村さんとログ見ながら雑談してたんだ'];
      return ['分析、見たよ。いいね', 'お、いいタイミング。お菓子食べる？', '中村さんからもらったクッキー、うまいんだよな', '次は利用率向上の施策を考えていこう'];
    }
    if (npc.name === '中村さん') {
      if (this.gameStep < 2) return ['新しいポータル、使ってます！', 'あ、よかったらこのお菓子どうぞ', '木村さんとも、最近よく感想を話してるんですよ', 'みんなにも聞いてみますね', '使いやすくなるといいなぁ'];
      if (this.gameStep === 2) return ['総務部のみんなにも聞いてみました', 'お知らせ機能は好評みたいです', '木村さんにも報告したら「助かるよ」って言ってもらえました', 'でも検索機能については…ちょっと声がありました'];
      return ['新しい機能、早速使ってみました！', '前よりずっと使いやすくなりました', 'お疲れさまです、よければクッキーどうぞ', '休憩中に木村さんとも話したんですけど、評判いいですよ'];
    }
    return npc.lines;
  }
  ...
  private openChoices(key: 'm1' | 'm2' | 'incident') {
    ...
    const title = key === 'm1' ? '📊 利用状況をどう分析しますか？'
      : key === 'm2' ? '📝 利用率向上の施策をどう考えますか？'
      : INCIDENT.title;
  ...
  update(_t: number, delta: number) {
    ...
    if (this.chapterClearShown) {
      if (Phaser.Input.Keyboard.JustDown(this.spaceKey) || this.virtualPad.isActionPressed()) {
        window.dispatchEvent(new CustomEvent('sier-chapter-cleared', { detail: { chapterId: 'easy-chapter6' } }));
      }
      return;
    }
