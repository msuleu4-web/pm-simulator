...
// ── スコアバランス計算メモ ───────────────────────────────────
// 各チャプター(1〜5)は「ミッション1 / ミッション2 / 炎上イベント」の計3つの
// 選択肢セットを持ち、各セットの選択肢スコアは共通で {+10, -5, +5}。
//   ・章ごとの最大: 10×3 = 30点 / 最小(NORMAL): -5×3 = -15点
//   ・5章合計の最大: 30×5 = 150点（bonusMultは全難易度1倍のため難易度に関わらず共通）
//   ・5章合計の最小(難易度別。penaltyMultは easy=0.5 / normal=1 / hard=2):
//       EASY:   round(-5*0.5)×3×5 = -2×15  = -30点
//       NORMAL:      -5      ×3×5 = -5×15  = -75点
//       HARD:   round(-5*2)  ×3×5 = -10×15 = -150点
//   ・全15セットで「無難（+5）」を選び続けた場合のベースライン = 5×15 = 75点
//
// エースSE到達ライン: 旧基準(80点)はベースライン(75点)から+5、つまり全15回の
// 選択のうち「最善（+10）」を1回選ぶだけで到達してしまい、サバイバー判定
// （75点から-36で到達）に比べて著しく到達しやすい非対称な設計だった。
// → エースSEのラインを100点に引き上げ。ベースライン(75)から+25必要となり、
//   「無難」を「最善」に5回置き換える＝平均すると各章で1回は最善手を選ぶ
//   必要がある、という現実的な達成難度になる（150点中100点=66.7%）。
export function getEndingTier(totalScore: number): EndingTier {
  if (totalScore >= 100) return 'ace';
